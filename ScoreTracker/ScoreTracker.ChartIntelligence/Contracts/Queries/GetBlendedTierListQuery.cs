...
namespace ScoreTracker.ChartIntelligence.Contracts.Queries
{
    ...
    ///     "Popularity", "Chabala", "PG", "PUMBILITY". Personalized bends the two lenses
    ///     that personalize — Score through the peer projection, PUMBILITY by reading the
    ///     viewer's own cohort's pool counts; when false the result is the shared
    ///     community reference (and is cached per folder+lens, not per user). UserId
    ///     defaults to the current user when Personalized.
    /// </summary>
