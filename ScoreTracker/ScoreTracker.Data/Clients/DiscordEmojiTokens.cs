...
{
    private static readonly IDictionary<PhoenixLetterGrade, string> LetterGradeEmojis = new Dictionary<PhoenixLetterGrade, string>
    {
    ...
    private static readonly IDictionary<PhoenixLetterGrade, string> BrokenLetterGradeEmojis =
    ...
    // Color is the mix signal at inline size: Phoenix = blue,
    // Phoenix 2 = green, XX = pink.
    private static readonly IDictionary<string, string> MixEmojis = new Dictionary<string, string>(
    ...
    private static readonly IDictionary<PhoenixPlate, string> PlateEmojis = new Dictionary<PhoenixPlate, string>
    {
        {
    ...
    private static readonly IDictionary<string, string> DifficultyShortHandEmojis =
    ...
    {
        var replacedMessage = LetterGradeEmojis.Aggregate(message,
        ...
        replacedMessage = BrokenLetterGradeEmojis.Aggregate(replacedMessage,
            (current, letterKv) => current.Replace($"#LETTERGRADE|{letterKv.Key}|True#", letterKv.Value,
                StringComparison.OrdinalIgnoreCase));

        replacedMessage = PlateEmojis.Aggregate(replacedMessage,
            (current, plateKv) => current.Replace($"#PLATE|{plateKv.Key}#", plateKv.Value,
                StringComparison.OrdinalIgnoreCase));

        replacedMessage = DifficultyShortHandEmojis.Aggregate(replacedMessage,
            (current, difficultyKv) => current.Replace($"#DIFFICULTY|{difficultyKv.Key}#", difficultyKv.Value,
                StringComparison.OrdinalIgnoreCase));

        replacedMessage = MixEmojis.Aggregate(replacedMessage,
