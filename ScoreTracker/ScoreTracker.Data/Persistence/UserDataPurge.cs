...
public static class UserDataPurge
{
        ...
        IReadOnlyList<Type> userOwned, Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        {
            var (bound, column) = Plans.GetOrAdd(entityType, t => (DeleteForMethod.MakeGenericMethod(t),
                UserKeyColumnOf(database, t)));
    ...
    /// <summary>
    ///     The Guid property naming the owning user. Convention-resolved so a vertical's manifest
    ///     stays a plain type list; a type carrying two candidates throws rather than guessing
    ///     which one means "whose data is this".
    /// </summary>
    private static string UserKeyColumnOf(ChartAttemptDbContext database, Type entityType)
    {
        ...
        var candidates = mapped.GetProperties()
            .Where(p => p.ClrType == typeof(Guid) && p.Name.EndsWith("UserId", StringComparison.Ordinal))
            .Select(p => p.Name)
            .ToArray();
        ...
        return candidates.Length switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"{entityType.Name} carries several user keys ({string.Join(", ", candidates)}); " +
                "purging it needs a hand-written repository rather than the shared manifest.")
        };
    }
}
