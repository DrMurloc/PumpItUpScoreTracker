...
namespace ScoreTracker.PlayerProgress.Contracts.Queries
{
    ...
    ///     peers the scores are read at (D51): the PUMBILITY page passes its chip, every other
    ///     caller reads Good (D50). The rung is applied when the cached sweep is priced, so the
    ...
    public sealed record ProjectPumbilityGainsQuery(Guid UserId, MixEnum Mix = MixEnum.Phoenix,
        ChartType? ChartType = null, Energy Energy = Energy.Good) : IQuery<PumbilityProjection>;
}
