...
internal sealed class EFUcsRepository : IUcsRepository
{
    ...
    public async Task<IEnumerable<UcsChart>> GetUcsCharts(CancellationToken cancellationToken)
    {
        ...
        var ucsEntities = await database.Set<UcsChartEntity>().ToArrayAsync(cancellationToken);
        ...
        var counts = (await database.Set<UcsChartLeaderboardEntryEntity>().ToArrayAsync(cancellationToken))
            .GroupBy(e => e.ChartId)
    ...
    public async Task CreateUcsChart(UcsChart chart, CancellationToken cancellationToken)
    {
        ...
        var entity = await database.Set<UcsChartEntity>().Where(u => u.Id == chart.Chart.Id)
            .FirstOrDefaultAsync(cancellationToken);
        if (entity == null)
        {
            await database.Set<UcsChartEntity>().AddAsync(new UcsChartEntity
            {
                Artist = chart.Artist,
    ...
    {
        await using var database = await _dbFactory.CreateDbContextAsync(cancellationToken);
        return (await database.Set<UcsChartLeaderboardEntryEntity>().Where(e => e.ChartId == chartId)
    ...
    {
        await using var database = await _dbFactory.CreateDbContextAsync(cancellationToken);
        var entity = await database.Set<UcsChartLeaderboardEntryEntity>()
            .Where(e => e.ChartId == chartId && e.UserId == userId)
            .FirstOrDefaultAsync(cancellationToken);
        if (entity == null)
        {
            await database.Set<UcsChartLeaderboardEntryEntity>().AddAsync(new UcsChartLeaderboardEntryEntity
            {
                ChartId = chartId,
                UserId = userId,
    ...
    public async Task<IEnumerable<ChartTagAggregate>> GetChartTags(CancellationToken cancellationToken)
    {
        ...
        return (await database.Set<UcsChartTagEntity>().ToArrayAsync(cancellationToken))
            .GroupBy(c => (c.ChartId, c.Tag))
    ...
    public async Task DeleteChartTag(Guid chartId, Guid userId, Name tag, CancellationToken cancellationToken)
    {
        ...
        var entity = await
            database.Set<UcsChartTagEntity>().FirstOrDefaultAsync(e =>
                e.ChartId == chartId && e.UserId == userId && e.Tag == tagString, cancellationToken);
        if (entity != null)
        {
            database.Set<UcsChartTagEntity>().Remove(entity);
            await database.SaveChangesAsync(cancellationToken);
        }
    }
    ...
    public async Task AddChartTag(Guid chartId, Guid userId, Name tag, CancellationToken cancellationToken)
    {
        ...
        if (entity == null)
        {
            await database.Set<UcsChartTagEntity>().AddAsync(new UcsChartTagEntity
            {
                ChartId = chartId,
                Tag = tagString,
                UserId = userId
    ...
    {
        await using var database = await _dbFactory.CreateDbContextAsync(cancellationToken);
        return (await database.Set<UcsChartTagEntity>().Where(c => c.ChartId == chartId && c.UserId == userId)
    ...
    public async Task<IEnumerable<UserChartTag>> GetAllMyTags(Guid userId, CancellationToken cancellationToken)
    {
        ...
        return (await database.Set<UcsChartTagEntity>().Where(c => c.UserId == userId)
                .ToArrayAsync(cancellationToken))
