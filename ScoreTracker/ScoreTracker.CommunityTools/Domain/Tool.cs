...
internal sealed class Tool
{
    ...
    public static Tool Create(Guid id, Guid ownerUserId, Name name, DateTimeOffset createdAt)
    {
        return new Tool(id, ownerUserId, name, null, null, ToolVisibility.Private, true,
            WebhookMode.None, null, Array.Empty<MixEnum>(), createdAt, null, null);
    }
        ...
        string? rejectionReason)
    {
        return new Tool(id, ownerUserId, name, description, url, visibility, acceptsAllToolsShare,
            webhookMode, webhookUrl, mixes, createdAt, approvedAt, rejectionReason);
    }
    ...
    ///         that individually and the ones already connected agreed to something else.
    ///     </para>
    /// </summary>
    public void SetWebhook(WebhookMode mode, Uri? url, int connectedPlayerCount)
    {
        ...
        WebhookMode = mode;
        WebhookUrl = mode == Contracts.WebhookMode.None ? null : url;
    }
}
