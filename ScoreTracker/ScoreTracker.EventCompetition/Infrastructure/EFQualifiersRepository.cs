...
{
    internal sealed class EFQualifiersRepository : IQualifiersRepository
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var entity =
                await database.Set<UserQualifierEntity>().FirstOrDefaultAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            await database.Set<UserQualifierHistoryEntity>().AddAsync(new UserQualifierHistoryEntity
            {
                TournamentId = tournamentId,
                Id = Guid.NewGuid(),
                Entries = entryJson,
        ...
        {
            await using var database = await _factory.CreateDbContextAsync(cancellationToken);
            var history = await database.Set<UserQualifierHistoryEntity>().Where(uqh => uqh.TournamentId == tournamentId)
                .ToArrayAsync(cancellationToken);
            var existing = (await database.Set<UserQualifierEntity>().Where(uq => uq.TournamentId == tournamentId)
        ...
        public async Task Restore(Guid tournamentId, string name, CancellationToken cancellationToken)
        {
            ...
            await using var database = await _factory.CreateDbContextAsync(cancellationToken);
            var history = await database.Set<UserQualifierHistoryEntity>()
        ...
        {
            await using var database = await _factory.CreateDbContextAsync(cancellationToken);
            var entities = await database.Set<UserQualifierEntity>().Where(e => e.TournamentId == tournamentId)
                .ToArrayAsync(cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            await using var database = await _factory.CreateDbContextAsync(cancellationToken);
            var config =
                await database.Set<QualifiersConfigurationEntity>().FirstOrDefaultAsync(e => e.TournamentId == tournamentId,
                    cancellationToken);
            if (config == null)
            {
        ...
        public async Task SaveTeam(Guid tournamentId, CoOpTeam team, CancellationToken cancellationToken = default)
        {
            ...
            var individualPlayers =
                await database.Set<CoOpPlayerEntity>().Where(p => playerNames.Contains(p.PlayerName)).ToArrayAsync(
                    cancellationToken);
            database.Set<CoOpPlayerEntity>().RemoveRange(individualPlayers);
            await database.Set<CoOpPlayerEntity>().AddRangeAsync(new[]
            {
            ...
            var existingTeam =
                await database.Set<CoOpTeamEntity>().FirstOrDefaultAsync(
                    t => t.TournamentId == tournamentId && t.TeamName == teamName, cancellationToken);
            if (existingTeam == null)
            {
                await database.Set<CoOpTeamEntity>().AddAsync(new CoOpTeamEntity
                {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var entity =
                await database.Set<CoOpPlayerEntity>().FirstOrDefaultAsync(
                    p => p.TournamentId == tournamentId && p.PlayerName == playerName, cancellationToken);
            if (entity == null)
            {
                await database.Set<CoOpPlayerEntity>().AddAsync(new CoOpPlayerEntity
                {
                    TournamentId = tournamentId,
        ...
        {
            await using var database = await _factory.CreateDbContextAsync(cancellationToken);
            return (await database.Set<CoOpPlayerEntity>().Where(c => !c.IsInTeam)
                    .ToArrayAsync(cancellationToken))
        ...
        {
            await using var database = await _factory.CreateDbContextAsync(cancellationToken);
            return await (from t in database.Set<CoOpTeamEntity>()
                    join p1 in database.Set<CoOpPlayerEntity>() on new { t.TournamentId, Name = t.Player1Name } equals new
                        { p1.TournamentId, Name = p1.PlayerName }
                    join p2 in database.Set<CoOpPlayerEntity>() on new { t.TournamentId, Name = t.Player2Name } equals new
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var existing =
                await database.Set<UserTournamentRegistrationEntity>().FirstOrDefaultAsync(
                    t => t.TournamentId == tournamentId && t.UserId == userId, cancellationToken);

            if (existing == null)
            {
                await database.Set<UserTournamentRegistrationEntity>().AddAsync(new UserTournamentRegistrationEntity
                {
                    TournamentId = tournamentId,
                    UserId = userId
                }, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await database.Set<UserTournamentRegistrationEntity>().Where(t => t.TournamentId == tournamentId)
                .Select(e => e.UserId)
                .ToArrayAsync(cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return await database.Set<UserTournamentRegistrationEntity>().Where(t => t.UserId == userId).Select(e => e.TournamentId)
                .ToArrayAsync(cancellationToken);
        }
    }
}
