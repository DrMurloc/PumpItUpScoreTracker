...
- {upgradedTitle.Key} {emoji}";
        ...
        public async Task Consume(ConsumeContext<UserWeeklyChartsProgressedEvent> context)
        {
            ...
            await SendToCommunityDiscords(context.Message.UserId,
                $"{user.Name} progressed to {context.Message.Place} on {chart.Song.Name} {chart.Song.Name} #DIFFICULTY|{chart.DifficultyString}# - {context.Message.Score} #LETTERGRADE|{context.Message.Score.LetterGrade}|{context.Message.IsBroken}# #PLATE|{context.Message.Plate}#",
                context.CancellationToken);
        }
    }
}
