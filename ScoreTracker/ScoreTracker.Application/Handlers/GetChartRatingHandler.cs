    ...
    GetChartRatingHandler : IRequestHandler<GetChartRatingQuery, ChartDifficultyRatingRecord?>
{
    ...
    {
        var rating = await _ratings.GetChartRatedDifficulty(request.Mix, request.ChartId, cancellationToken);
        if (!_currentUserAccessor.IsLoggedIn || rating == null) return rating;

        var myRating = await _ratings.GetRating(request.Mix, request.ChartId, _currentUserAccessor.User.Id,
            cancellationToken);
