...
- **bUnit event seam**: a component test that asserts on the *result* of an event awaits the dispatch — `await element.ClickAsync(new MouseEventArgs())`, not `element.Click()`. The synchronous helpers post the event to the renderer's dispatcher and return without waiting for it, so under parallel load the assertion on the next line reads the pre-event render: no exception, a valid `blazor:onclick` in the markup, and the component's state simply not yet written. Ratcheted shrink-only by `BunitEventDispatchTests`; the suite's `ThreadPoolWarmup` keeps the un-converted backlog survivable but widens the window rather than closing it.
- **bUnit service seam**: a component test that needs a stubbed dependency adds a setup to the mock `ComponentTestBase` already registered (`Mediator`, `CurrentUser`), never a second registration of its own. bunit freezes its service provider the first time anything resolves from it — which a render does — so a later `Services.AddSingleton` throws `InvalidOperationException` rather than overriding, and the message points at the registration instead of at the render that closed the provider.
