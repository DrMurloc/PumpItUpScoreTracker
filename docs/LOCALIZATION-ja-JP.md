...
- **`チャート` is drifting back in (17 values as of the 2026-07-26 QC pass).** The rule is `Chart` → 譜面 and the file is 238-to-17 in its favour, but new batches keep reaching for the loanword. The newest offender (the Life Calculator's rainbow-bar explainer) was swept to 難しい譜面 in that pass; the rest are older strings and want **one dedicated sweep**. Watch the search/admin cluster in particular (`Find a chart`, `Bulk Add Charts`, `No charts match your filters`, `Dismissed charts`, `This week's charts`).
