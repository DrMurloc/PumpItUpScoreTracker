...
| Writes | Yes | **Two**, both personal token only: `POST api/v2/players/me/plays` records an observed play, and `POST api/v2/players/me/sittings/close` ends the caller's open sittings on a mix. Every other mutation stays on v1 with a personal token |
...
| Players | `api/v2/players` | Who shared with you; `{id}`, `{id}/scores`, `{id}/sessions`, `{id}/journal`. `me` works with a personal token. **Additive 2026-08-17:** `judgments` (on scores and journal entries) carries `maxCombo` — solved from the score and the five counts, `null` when unsolvable; journal entries carry `isStageBroken` — a play the stage interrupted, `isBest` false, `score` null ([stage-breaks-and-max-combo.md](design/stage-breaks-and-max-combo.md)). **Additive 2026-09-05:** `{id}/scores` takes `chartIds` — a comma-separated list of at most 50 chart ids, the point read for one player on one chart; the list takes `community=<name>` (World, a country, or a community's own name), which only ever narrows the players you can already read and filters a private community only for its members; `{id}/stats` and the bulk `stats` carry a player's PUMBILITY numbers per mix — the merged, singles, doubles and co-op pools, competitive levels, highest level, clear count and the estimated place on piugame's official ranking — highest PUMBILITY first in the bulk, 404 on a legacy mix ([api-v2-round-2.md](design/api-v2-round-2.md)). **Breaking change 2026-09-23 (owner call):** `{id}/sessions` groups the way the Sessions page does — a player's imports in one mix until eight hours pass without one — so several imports in one night are one session rather than one each; `sessionId` is the newest stored session in the group and the new `sessionIds` lists every one folded in, so the `sessionId` on a journal entry or a score-push webhook always appears in exactly one session; and the list no longer stops after a player's newest 50 sessions ([session-breakdown.md §8](design/session-breakdown.md)) |
| Weekly charts | `api/v2/weekly-charts` | The current board and scores on it |
| Plays (write) | `POST api/v2/players/me/plays` | **New 2026-09-22, the first write on v2:** a play the caller observed — mix, chart (`chartId`, or `songName` + `chartType` + `level`), the five judgment counts, `maxCombo`, `score`, `isBroken`, `playedAt`, a `source` naming the capturing tool and an optional `recordBrokenAsBest` (the mix's default when omitted). The server recomputes the score from the judgments and answers `400` when they do not reconcile (the judgment checksum), derives the award from them and refuses a claimed award they do not earn; a play that reconciles lands in the journal and, when it beats the record, becomes the personal best. Personal token only; a mix must be `phoenix`-scored. Plays gather into *sittings* per player and mix — a play more than 15 minutes from every open sitting's plays starts a new one — and each sitting posts one Discord session card once 15 minutes pass with nothing new arriving, or as soon as the caller closes it (the Sittings row below) ([rise.md §12](design/rise.md)). **Changed 2026-09-26:** on `Rise` and `RiseArcade` those 15 minutes are 4 hours, a fallback for a capture app that never got to close its sitting ([rise.md D25](design/rise.md)). Built for the RISE capture app, open to any tool ([rise.md §6.3](design/rise.md)). **Additive 2026-09-23, with one exception:** the five counts and `maxCombo` are optional and travel together — all six or none, `400` `judgments-incomplete` otherwise. A play without them is recorded as read: no checksum, and an optional `award` is taken as sent provided the score agrees with it (only `1000000` is a Perfect Game); a million with no `award` records a Perfect Game, and a pass without judgments must score above zero. A play with them is checked as before. The exception: a count left out no longer reads as zero, so a client whose serializer drops zero-valued fields has to send them ([rise.md D24](design/rise.md)) |
| Sittings (write) | `POST api/v2/players/me/sittings/close` | **New 2026-09-26:** ends the caller's open sittings on a mix — body `{ "mix": … }`, required — and announces each one the way a sitting that went quiet does: the Discord session card, highlights, folder lamps. Answers `204` whether or not anything was open, and a repeat never announces anything twice; the next play on that mix starts a new sitting. It closes whatever is open when it lands, so a tool sends it in order with its plays: after the session's last play has been answered, before the next session's first. A retried close that goes out after the next session's first play closes that session too. Personal token only; the mix must be `phoenix`-scored. Built so the RISE capture app decides when a session ends ([rise.md D25](design/rise.md)) |
