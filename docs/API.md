...
| Charts | `api/v2/charts` | Charts for one mix; `{id}`, `{id}/similar`, `random`. Carries `scoringLevel` — how hard it is to *score* on, null where unmeasured. **Additive 2026-09-05:** `{id}/scores` — every readable player's best on that chart, passes first then failed bests, each row the per-player score row with `userId`, `username` and `gameTag` in front. Share-gated exactly like `/players`: a tool sees the players who shared with it, a personal token its own user, and nobody else is on the page ([api-v2-round-2.md](design/api-v2-round-2.md)). **Additive 2026-09-12:** every chart row carries `version` and `releaseDate` — the patch of *this* mix the chart first appeared in (a carry-over reads the mix's launch version; both null when unknown) — and `charts`, `charts/skills` and `charts/random` take `releasedInVersion` (one or a comma list), `releasedByVersion` (up to and including), `releasedAfterVersion` (strictly after) and `releasedAfter` (a date, exclusive); the version filters AND together and never match an unknown version; an unknown name is a `400` pointing at `versions` ([chart-versions.md](design/chart-versions.md)) **Additive 2026-09-13:** every chart row carries `debut`, true when this mix is the chart's debut mix (so `version` is the patch that introduced it), false on a carry-over. |
| Versions | `api/v2/versions` | **Additive 2026-09-12:** a mix's patches oldest first — `name`, `releaseDate` (null on an undated legacy patch), `sortOrder`, `chartCount` (charts first released in that patch, in that mix). The value every `released*Version` parameter takes ([chart-versions.md](design/chart-versions.md)) |
