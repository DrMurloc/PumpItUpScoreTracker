...
| Sittings (write) | `POST api/v2/players/me/sittings/close` | **New 2026-09-26:** ends the caller's open sittings on a mix — body `{ "mix": … }`, required — and announces each one the way a sitting that went quiet does: the Discord session card, highlights, folder lamps. Answers `204` whether or not anything was open, so a retry is safe; the next play on that mix starts a new sitting. Personal token only; the mix must be `phoenix`-scored. Built so the RISE capture app decides when a session ends ([rise.md D25](design/rise.md)) |
