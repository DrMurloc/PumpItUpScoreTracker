...
on that line: every rung of the ladder in scope in a list, the merged one grouped by gem, plus an **×** on a [P.B]
level that drops back to the gem around it. It **starts on the band that was read**, so a viewer who has chosen
nothing is already on their own — there is no separate *mine* entry, because picking your own band back is picking
it (owner, on the PR). Inside a gem's group the heading already says which gem it is, so the lines read **All
Levels**, **Level 1**, **Level 2** rather than spelling the gem out six times. The choice is remembered as
