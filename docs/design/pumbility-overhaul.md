...
capped at the 99th percentile of what peers keep or at your own highest, so a few one-folder pools cannot
flatten the boxes; a line that runs past the top ends in a caret. A level gets a column when one peer in
fifty keeps a chart there, or you do. Each column's tooltip names the count under the pointer, the median
...
| Layer | Change |
|---|---|
| **Domain** | `PeerLevelSpread` (pure, `Domain/Services`, beside `PumbilityPoolSplit`): `Of(PeerPoolSummary, levels, mine)` counts every peer's fifty per level and answers the peers, the board peers and one `LevelSpreadColumn` per level, from the lowest to the highest that one peer in fifty keeps or you do — how many peers keep each count, fewest, the quartiles and the median, most, how many keep at least one, your count, and how many peers keep fewer than you and exactly as many |
