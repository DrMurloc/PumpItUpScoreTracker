...
- **I4 — Phones stop side-scrolling**: the Rankings board joined the sanctioned density preference (`Density__OfficialRankings`, default Table) — Comfortable/Compact render card rows (rank + delta, player, rating with a boards caption in Comfortable, Top 50 expander via the shared `HubRankingsTopCharts` body) with `MudPagination`, while Table mode keeps the pinned-column grid, stacks the week delta under the rank number, and truncates the pinned player earlier so the rating column keeps its width. This Week's movers and climbers rows stack their tail (rank path / value, places / boards) vertically under 700px.
