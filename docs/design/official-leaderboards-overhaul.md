...
2. **Rating boards** — the mix's PUMBILITY board(s), paged until end: P2 splits it into All/Singles/Doubles tabs with decimal cents and gates the whole area behind the service login; P1 serves one whole-number board of 1000 on a single page to an anonymous session, ignores the `?t=` tab, and additionally still publishes its per-level rating lists (mirrored alongside). Upsert dims, append placements. Olympic tie places computed exactly as today.
...
- **G5 — Section of pages**: the tabbed hub became five routed pages (`/OfficialLeaderboards` + `/Rankings` + `/Players` + `/Popularity` + `/WhatItTakes`) sharing `OfficialSectionFrame` (title, import-health strip, section nav, mix/chart context). The desktop Compete menu grew an Official Leaderboards mega-column and the mobile More sheet a matching disclosure; tab-era `?view=`/`?player=` links redirect onto the real pages, `/PlayerRankings` aliases the Rankings page, and `?player=` deep links live on `/Players`.
- **H1/H2 — Popularity depth settled**: live recon proved the sweep's ajax endpoint (the site's own More-button call) serves the whole ranked catalog — 4,037 charts on Phoenix 2, every tile parseable — and the walk now terminates on the endpoint's raw tile count instead of parse survivors, so a drifted tile can't silently truncate the tail. The song ranking's twin endpoint (`/ajax/top_songs.php`) is confirmed and unmirrored.
- **H4 — The popularity page top**: Hottest/Coldest Charts trios straight off the board's ends, a Biggest Movers wrap-strip (movement-ranked, debuts starred, collapses on baseline weeks), and Folder heat — the median chart's board percentile per level as Singles/Doubles lines in the home-page graph vocabulary (`ApexChartTheming.BaseOptions`, `ChartTypeHex`, reversed y so up = hotter, nothing scrollable). Highlights render only on the unfiltered charts view; the caption counts the ranked board.
- **H5 — Popularity round two**: Folder heat became two overlapped P25–P75 shaded regions with smooth median lines (the By-Level widget's range-area machinery, incl. its hand-built tooltip for the range-area/line collision); the chart list became a labeled, paginated **Leaderboard** table (full board, 50/page) with Song Artist and Step Artist columns (hidden on phones) and a per-row info icon opening the shared `ChartDetailsDialog`. `FolderGrid`/`FolderPicker` grew a `ShowCoOp` switch — the popularity board has no co-op rows, so the tab hides here. The Charts/Songs toggle now fills its active side, and folder controls hide (and the filter no-ops) on the Songs view.
- **I2 — The This Week hero**: the signed-off hype header above the tables — a pulse strip (new/upscored chart-board entries, players active, world-first count), four snapshot cards (biggest PUMBILITY *value* gainer with rank chip, highest-level world first, most chart-board activity, and the #100/#1000 PUMBILITY floors as 50× SS levels with week deltas), a debuts strip (first-ever appearance on any board, chips deep-linking into Players), and a template-composed hype sentence. All sweep-computed editorial: `HighlightsCalculator.HeroSummary` stores the new playerless kinds (PlayerId went nullable), debuts judge against an all-history seen-set the rebuild accumulates as it replays, and the floor levels mirror the What It Takes cutline exactly (Singles, SG plates, `PumbilityScoring(mix, false)`). The hero hides until a pulse row exists — Rebuild weekly highlights materializes it for the current week.
- **I1 — First live-data round**: world firsts became cross-mix (`CrossMixRecordHighs` feeds `HighlightsCalculator`; a band already claimed on the chart in any mix falls back to a plain new #1 — Rebuild weekly highlights retro-cleans); official tags normalize to the whitespace-free board form at every seam and a data migration merged the account-page/board-scrape twins; bare "boards" copy became "chart boards" across the section and the digest; the Discord world-first line became bubble emoji + song + World First + grade emoji + linked player; the Players and What It Takes graphs moved onto `ApexChartTheming.BaseOptions` (frozen canvas, no vertical gridlines). Riding along: `PhoenixScore.LetterGrade` and the parameterless floor helpers were deleted repo-wide — every score→grade boundary names its mix (`LetterGradeFor`), and `RecapPlayerTypeCalculator` pins its archetype cutoffs as raw tuned scores.
- **I3 — Live-data rounds two and three**: climbers count what happened — entering a board credits N−P+1 places, **net places leads everywhere** (board count breaks ties), the eligibility minimum is gone, and every surface shows the entered/climbed split; PG ties never list as New #1. World firsts dropped the SS-and-up gate: **any band crossing counts** (`GradeBandLadder` grades each historical score in its own mix's table, so `CrossMixRecordHighs` compares band ranks, not raw scores across re-cut floors) and the hero card spotlights the highest-level one. Every debut stores its own row — the strip's "+N more" expands the whole class, however big the first weeks run. Archetype chips render outlined in their grade-ramp hue (Pass Pusher AA bronze, Perfectionist glowing); the player sheet leads with the difficulty bubble and sorts on every column.
- **I4 — Phones stop side-scrolling**: the Rankings board briefly grew the three-mode density toggle, then the field test picked a winner — **compact card rows are the only rendering** (owner: "Compact just looks straight up better"): rank + week delta, player, the archetype chip, rating, and the Top 50 expander (shared `HubRankingsTopCharts` body) with `MudPagination`; the boards-count column came off entirely (owner: not as useful as it looked). The UX note landed as rule 5's addendum: entity lists that would otherwise be tables lean compact-only; real tables stay for sortable grids (the player placements sheet, now ordered Place, Δ, chart, score, grade) and numeric matrices (What It Takes). This Week's movers and climbers rows stack their tail vertically under 700px.
- **I6 — Names, not tags; phones stop overflowing**: board tags are `TAG#1234`, but the digits identify an account rather than naming anyone, so **every list prints the human half only** (`HubPlayerChip.NameOf`, with the full tag kept in the row `title` and rendered in full on the player's own page via `ShowTag`). Names truncate at 16ch (11ch on phones). Song titles fall back to their jacket + difficulty bubble under 760px on the popularity board and This Week's world firsts — the jacket became the tap target so the chart-board dialog is still one tap away — and the world-first row stacks claimant over score there, matching the movers/climbers tails (both now stack from the frame's one mobile block). The popularity board dropped its pinned columns entirely: with the title gone the row fits, and the pinned cells' opaque backgrounds were reading as a card on a board that is deliberately flat. What It Takes keeps a frozen header row so the grade columns stay labelled down the full ladder.
- **I5 — Chart boards from anywhere + the glow**: `GetOfficialChartBoardQuery` serves any chart's full mirrored board at the latest seal; `OfficialChartBoardDialog` (compact rows, 10-then-more paging, per-mix grade icons) opens from every song name on the Players sheet and the This Week world firsts / New #1s. Rows carry the dashboard glow vocabulary (`--daily-you` blue / `--daily-community` green via `CommunityGlowReader`) — and so do the rankings cards and This Week lists, keyed on the mirrored player's linked `UserId`. The Players page auto-selects: your linked board tag (`GetLinkedOfficialPlayerTagQuery`) when signed in and linked, otherwise the top of the combined ranking board.
- **J1 — Phoenix has a PUMBILITY board too**: the sweep only ever asked Phoenix 2 for `pumbility_ranking.php`, so Phoenix's own board — one whole-number list of 1000, served anonymously on a single un-paginated page, tab parameter ignored — was never mirrored, and every surface keyed on the board name fell back or went dark: Movers said "no PUMBILITY board on this mix", Rankings ranked by computed rating, What It Takes had no ladder. Both mixes now take the PUMBILITY path, Phoenix keeping its per-level rating lists alongside. What It Takes compares only the boards a snapshot actually holds, so Phoenix's picker shows one board rather than two empty per-type tabs. Movers need two Phoenix imports to appear — the first mirrors the board, the second gives it a previous week to diff.
- **J3 — Every Phoenix 2 avatar was the default**: the mirror's `ImageRegex` only knew `/data/avatar_img/`, but Phoenix 2 serves avatars from `/data/avatar_img2/` — so every P2 avatar missed the match, `ConvertPiuGameAvatarToPiuScoresAvatar` returned null, and the board sweep's `?? DefaultAvatar` painted the stock art on the whole leaderboard (the P2 account-import profile image failed the same way). Widening the pattern is only half of it: the two directories **reuse ids for unrelated pictures** (verified against the live site 2026-07-26 — `avatar_img/9516….png` and `avatar_img2/9516….png` are different images entirely), so a shared `/avatars/` mirror would serve whichever mix imported an id first to both. P2 now mirrors into `/avatars/p2/`, the same split the site uses for stepballs. Purely additive: no P1 path changes and nothing re-copies, and the next P2 sweep overwrites the stock avatars because `EnsurePlayers` takes any non-null incoming avatar. The **personal P2 import** shared the same miss with a quieter symptom — the saga skips the `ProfileImage` write on a null avatar and `UpdateUserGameProfileHandler` coalesces back to the stored one, so a Phoenix 2 import simply never refreshed your picture (it did not corrupt it). Both entry points are pinned now. The *parsing* half was never broken: `PiuGameApi`'s own regex has accepted `avatar_img2` since the P2 rollout, with approval fixtures on both the board and account-data pages — the two regexes drifting apart is the recurring shape of this bug.
- **J4 — Refresh rating boards (ONE-TIME, delete after use)**: J1 and J3 both repair a stage that already ran, and waiting a week for the next sweep was not an option for a social push. `RefreshRatingBoardsCommand` re-scrapes the rating boards alone onto the **latest sealed snapshot** — the `RefreshPopularityCommand` shape, so no second import appears and the rows belong to the run that genuinely produced them. Three things make it safe to press: the scrape completes before anything stored is touched (a fetch that dies part-way leaves the snapshot untouched and the press re-runnable), `DeleteRatingPlacements` scopes itself by joining the leaderboard dimension so chart placements are never in range, and the consumer evicts the hub's snapshot-keyed cache entries by hand — keying on a snapshot id normally cannot stale, but writing *more* rows onto a sealed one is the exception, which is why those key formats moved into shared `OfficialCacheKeys`. It also carries the PUMBILITY pages' avatars through `RatingBoardEntry` into `EnsurePlayers` (null means keep, never default), which closes the standing gap where rating-board-only players arrived with no avatar. **Follow-up: delete the command, its consumer, `DeleteRatingPlacements`, the button and the test region once both mixes have been run.**
- **J2 — Bands are pictures**: This Week's world firsts and What It Takes' grade columns drew their bands as coloured text chips while the rest of the site drew letter art. `GradeBandIcon` renders a ladder band as the game's own art — the plate for a perfect game, the letter image below it — and What It Takes' ladder chips and column heads use `LetterGradeIcon` directly.
- **I7 — The import-health strip comes off the public frame**: G5's health chip hung the sweep's raw exception off a tooltip on every page in the section, so a failed Phoenix 2 run painted a .NET SSL stack trace across the header. Diagnostics are the maintainer's: the strip is gone, `OfficialSectionFrame` no longer sends `GetImportRunsQuery`, and the runs table on `/Admin/OfficialLeaderboards` remains the one place a stage+error is readable. Players keep the Last Updated stamp, which already says the data is stale. `DiagnosticExposureTests` ratchets it — the diagnostic-bearing contracts may only be referenced under `Pages/Admin/`.
