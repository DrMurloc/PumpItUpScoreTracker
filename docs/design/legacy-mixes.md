...
- **PR 1 — catalog corrections (DELIVERED 2026-07-11, awaiting owner run):** `Downloads\pumpout-catalog-corrections.sql` — 1,984 idempotent single-row UPDATEs (artists, step artists, BPM; pumpout-wins policy), one `XACT_ABORT` transaction, prod GUIDs, schema-independent — safe to run today. Note: a few Japanese artists lose native script (pumpout romanizes; our old values had embedded newlines/mojibake).
- **PR 2 — mixes exist:** MixEnum + MixIds + Mix rows/columns + `MixSelector` + "Debuted in" + OriginalMix backfill script. Display-only; nothing playable changes.
- **PR 3 — mixes are playable:** `LegacySlot` + `PlayerCount` + bubble legacy branch + ChartMix backfill (+cut-content Song/Chart rows) + `BestAttempt.MixId` + mix-aware recording/CSV upload. Commit the pumpout extractor alongside.
