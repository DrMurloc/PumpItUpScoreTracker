...
#### 8a-addendum. As-built (crawler PR, 2026-07-11) — the site was rewritten

Fixture capture on day one found piucenter rebuilt as a static-hosted SPA (SolidStart), which invalidated the HTML-crawl plan in its favor:

- **All data is static JSON** under `/chart-jsons/<version>/` (version stamped per data release, hardcoded in the site's `data-*.js` bundle — the client resolves it shell → bundle → regex and re-resolves when a page-content fetch comes back as HTML). No HtmlAgilityPack; `PiuCenterApi` is a JSON client in `Data/Apis`. Missing files return **HTTP 200 with the app shell**, so existence is content-sniffed, never status-coded.
- **Enumeration is one GET**: `page-content/chart-table.json` lists every chart, all packs (4,414 in release 050726) with their key, type, level, debut pack, top-3 skills, NPS, BPM, and sustain numbers. The alias seeding became an offline diff (tiered normalization: song+artist+type+level+variant, diacritics folded, HALFDOUBLE suffixes relaxed onto our Double charts) — 4,411/4,426 eligible Phoenix charts auto-matched, 15 NotFound (their licensing gaps), delivered as an idempotent SQL seed script. Key format is now `<Song>_-_<Artist>_<S|D><level>_<SUFFIX>` with INFOBAR marker tokens on some keys.
- **The old ~35-column numeric matrix is no longer published.** What we bank per chart (`ChartSkillMetric`): their top-3 dominance pick, per-segment skill-badge **coverage fractions**, last-segment badges + a last-segment-is-peak flag, rare-skill counts, per-skill practice-list ranks, NPS/sustain/time-under-tension, and their numeric difficulty prediction. Coverage fractions play the frequency role in every formula that wanted one.
- **The flip policy** (`PiuCenterSkillMapper`, calibrated on the full corpus vs the archived hand tags): top-3 → highlighted tags; coverage ≥ per-skill threshold (default 0.30; jack .40 / jump .50 / run .40 / twist_90 .40) → contains tags; Fast/Slow from NPS quartiles within (type, level); EndRun only when the final segment is the chart's hardest and carries a run badge on a non-run-dominant chart. Their 32-skill vocabulary maps many-to-one onto ours; `Skill.Runs` added; **doublestep/side3_singles stay metrics-only** (mapping them made Technical an umbrella over 76% of the catalog); **Gimmicks/VeryFast/Moderate retire functionally** (never emitted — enum members linger until a post-flip cleanup so pre-flip rows still parse).
- **Personalized blend (K7)**: the skill source pools score deviations across ±3 folders (decay 1.0/0.6/0.3/0.15) weighted by coverage via the Catalog's published `GetChartSkillChipsQuery` — fixing the cold-start hole where a player entering a new folder got no skill inference.
- **Bonus**: their table covers XX and every legacy pack — per-mix skills for older mixes are one capability-flag flip away once wanted.
...
**Round 14 (2026-08-13, above-the-fold rebalance):** the owner's mobile verdict — too noisy above the fold — plus 1:1/tablet rebalancing, mocked at five widths and locked in one pass. The sticky toolbar now owns the whole "what list am I looking at" choice at every width: **Ranked-by / Shown Difficulty (and Grouped-by) move up from the content bar and the Advanced drawer into the toolbar row** — which round 6's own principle already argued for, since the lens changes the data. Under 960 the **view buttons hand over to a View select** (one control, one line, locale-proof; reuses the existing "View" key); under 600 the lens controls take their own full-width toolbar line, so a phone reads as two clean select rows. **The round-9 Advanced drawer and its button are deleted** (superseded, not lost — everything it held is visible now; the "Advanced" L-key stays, ToolSetupWizard resolves it). **The badge-chips row is deleted** — it existed to name selects the drawer hid; the **Provisional warning chip survives**, leading a new context caption directly under the toolbar that also carries the demoted cohort note ("Ranked against N players…", caption-size) and the "How is this personalized?" link, moved off the view group. **Folder Stats and Download become icons in the content-bar tools cluster at every width** (tooltip + aria-label carry the names; the desktop toolbar's second row dies, so the sticky bar is one row everywhere ≥600). Width bands, re-cut same-day (owner: keep the four views glanceable as long as they fit): **<500** = folder+View select / lens rows; **500–959** (tablet portrait, 1:1, landscape phones) = folder + lens select(s) on line one with the **view button group taking the second line whole** — ~460px covers the longest locale (French), and 500 is also exactly where the My Scores pair of selects stops fitting beside the folder pill; **≥960** = buttons + lens select, one row. A side effect worth keeping: the thin-cohort "Not enough players" tooltip (a button `title` no select item can carry) now reaches every width ≥500. The "MyProgress" select value is a sentinel mapped before `ChangeGrouping` so the persisted grouping vocabulary stays real groupings only (`ViewPersistenceTests`).

**Round 13 (2026-07-26, cross-mix outline toggle):** the dashed-green "passed in another mix" outline becomes a **per-player switch** — "Show Passed in Another Mix", last in the Filters drawer's Display group, next to the other signed-in-only switch. Unlike the column switches it is **never disabled by density**: a border fits on a Compact jacket where a column never would. Off drops the outline in all three densities *and* its legend item (a legend entry for a border on no card is noise), and skips `GetCrossMixPassesQuery` on folder loads — so flipping it back on inside the same folder backfills the set the load never fetched. Defaults on; lives in the page settings blob (`ChartSkills__Settings`), not a standalone key, because it is a Display switch.

**Round 12 (2026-07-11, final polish — C21):** every drawer (Folder Stats, Filters, Advanced) gets an **explicit close button** — the click-away overlay is unreliable on mobile. The dialog's recording row moves to a **grid** with a trailing ghost column, so Broken stays left-packed next to Plate instead of gluing to Save; below 600px it stacks 2×2 (Score/Plate, Broken/Save). Owner: these are the last items for the tier-list page.

**Round 11 (2026-07-11, dialog recording rework — C20):** the "quiet recording" toggle is gone — the score inputs are **always visible** for signed-in users, prefilled from the current best. `EditChartGrid` is out of the dialog entirely (it brought a duplicate To-Do button, a favorites button, and a layout that collapsed on phones); the dialog owns a purpose-built row — Score, Plate, Broken, Save — dispatching the same `UpdatePhoenixBestAttemptCommand`, refreshing the journey timeline in place, wrapping cleanly at narrow widths.

**Round 10 (2026-07-11, details-dialog fixes — C19):** the round-7 active-lens meta row **double-printed** the active lens (the placements chips already name every lens's tier) — removed, along with its params. The **Popularity placement chip spoke difficulty** ("Popularity: Very Hard") — it now uses the popularity bucket vocabulary ("Very Unpopular" etc., same names as the page's Popularity sections). The **actions row collapsed into vertical letter-stacks on mobile** (spacer + shrinking buttons) — buttons are nowrap and wrap to their own line instead, and the PIU Center link sits on its own line below.

**Round 9 (2026-07-11, compact-mode redesign — C18):** round 8's bottom action bar stacked awkwardly on the shell's bottom nav — **deleted**. New compact model (<960): the toolbar is **one clean row — Folder → view buttons → "Advanced"**; the **Advanced drawer** holds, in owner-specced order, **Ranked-by → Grouped-by → Personalized → Download**; **Folder Stats moves to the Ranked-by slot in the content bar** (the lens controls only render there on desktop). **The Filters icon + drawer are width-independent** — filtering always lives at the content bar's top right, never inside Advanced (two separate drawers by design). Below **600px** the Advanced button reorders before the view group and the toolbar centers, so the wrap lands deliberately as folder + Advanced / views (the view buttons were wrapping raggedly ~450, language-dependent). A shared `lensControls` RenderFragment keeps the desktop content bar and the Advanced drawer from drifting. Razor gotcha recorded: inline `@<text>` template assignments parse only in code mode — after markup siblings they need an explicit `@{ }` block.
