...
| Version | The patch of the mix in view the chart first appeared in (`Chart.AddedIn`, [chart-versions.md](chart-versions.md)): chips per patch with counts, grouped by major version, a *Through a version* bulk select, All/Clear; URL key `Version=` carries the exact set; hidden on a mix with one version; unknown reads as absence. *Newest content* sorts by release order within the mix, then debut era. Export columns `Version`, `ReleaseDate`. |
| Channel | The song's channel on the mix in view (`Song.Channel`, [song-channels.md](song-channels.md)): chips in the game's order with chart counts, any-of; URL key `Channel=` carries the exact set of enum names; hidden while the mix has no channel rows; unknown reads as absence. Export column `Channel`. |
