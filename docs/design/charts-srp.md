...
| Version | The patch of the mix in view the chart first appeared in (`Chart.Release`, [chart-versions.md](chart-versions.md)): chips per patch with counts, grouped by major version, a *Through a version* bulk select, All/Clear; URL key `Version=` carries the exact set; hidden on a mix with one version; unknown reads as absence. *Newest content* sorts by release order within the mix, then debut era. Export columns `Version`, `ReleaseDate`. |
