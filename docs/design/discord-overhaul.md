...
| 4 | The hardcoded weekly-progression channel post (`WeeklyTournamentSaga` → channel `1254418262406725773`) is **deleted, replaced by nothing**. It only fed the owner's test chamber. The weekly progression event keeps publishing (PlayerProgress capture and the site UI consume it); only the Discord send dies. Since renamed `UserWeeklyChartScoreImprovedEvent` (weekly-charts-overhaul.md §9.5). |
