...
| D39 | **The page must never guess that capture has FINISHED.** An import is many batches over minutes and each runs capture separately, so the row count sitting still for a few seconds means "between batches" exactly as readily as "done". Three attempts to read completion out of side effects each failed differently: the first row arriving (cleared mid-pipeline), a stable count (cleared between batches), and gating the watcher on the card (never watched at all when a page opened mid-pipeline). The watch instead runs for as long as the **window** is open and simply refreshes — no completion guess exists to be wrong. The window costs nothing to keep honest because it hangs off `LastActivityAt`, which every batch advances: a live import extends its own deadline, and the watch outlives the last batch by the window. |
| D40 | **⚠ The real fix is a stamp, and it is owed.** Three bugs have come out of inferring capture state, and all three are impossible with a `HighlightsCapturedAt` on `ScoreSession`: the terminal event already exists, nothing persists it. The test is **`HighlightsCapturedAt >= LastActivityAt`** — anything less is per-batch and says nothing about the import as a whole. `ScoreLedger` may consume `ScoreHighlightsCapturedEvent` directly (it sits downstream of PlayerProgress), so this needs a migration and one consumer, not a port. It would also separate *failed* from *unremarkable*, which render identically today even though the capture saga swallows each step's exceptions. Deferred because it is a migration — not because the inference is adequate. |
