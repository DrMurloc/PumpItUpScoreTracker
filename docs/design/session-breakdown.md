...
| D39 | **"Started" and "finished" are different questions, and the page needs both.** Capture is not one write — it lands flags, then folder lamps, then the rating step that produces the competitive baseline and the PUMBILITY gain, then titles, and only then publishes `ScoreHighlightsCapturedEvent`. D38's inference answers *started*, which is all a first read can know. **Finished** is answered by the watcher instead: the card holds until the captured-row count stops moving, because writes that have stopped arriving are writes that are done. Treating the first row as proof of completion cleared the card over half-written data and showed a session stripped of its decorations — the exact failure this branch exists to prevent. |
| D40 | **⚠ The real fix is a stamp, and it is owed.** Two bugs have now come out of inferring capture state from its side effects, and both are impossible with a `HighlightsCapturedAt` on `ScoreSession`: the terminal event already exists, nothing persists it. It would also separate *failed* from *unremarkable*, which render identically today even though the saga swallows each step's exceptions. Deferred only because it is a migration — not because the inference is adequate. |
