...
| 12 | **Channel filter** (2026-09-13, [song-channels.md](song-channels.md)): channel chips beside Song Types, in the mix's own channel set — nothing picked draws from every channel; the preset stores the exact set of channel names, so a "K-Pop night" preset stays K-Pop after the next patch. `RandomSettings.Channels`; `charts/random` takes the API's `channel` parameter. The row hides on a mix with no channel rows. |
