...
| 5 | **List-boards: Hardmode and themes** | `LeaderboardBoard` / `LeaderboardBoardChart` (D45); the roll copies that day's Hardmode list into the season's board; the `ThemeMatcher` kinds, the console's matcher editor, dry run and per-season activation (D46); the chart-tag table and its console (D47; tagging Gimmick is the owner's data entry afterwards); the Stamina rule iterated in the dry run against the local catalog until it reads right or becomes a finite list (a build step, §14); list-board standings from the stats fact and the rebuild; the section's Hardmode page in seasonal view and the theme pages; the PUMBILITY page's Hardmode tab in seasonal view with the locked note (D30). | Pick the first two or three themes here. Activate a Banya theme for the running season: its list freezes, its board fills after one rebuild, a Banya song added to the catalog afterwards does not appear. The season's Hardmode list does not change when the weekly census rewrites the all-time one; your season Hardmode standing on both tabs agrees. | Slice 3 merged. |
...
| 8 | **CO-OP levels and CO-OP PUMBILITY** | Two halves. First: the anchoring probe (the pass-anchored and the score-anchored level for every co-op, printed side by side), `CoOpDifficulty` (D42) seeded from it, the console pin, the season and official configs pricing a co-op on the Doubles curve at its level when one exists, the CO-OP PUMBILITY board (D41) in both views re-priced by the chart-difficulty event — the board can go live on seeded and hand-pinned levels, because the table is the truth. Second, after the checkpoint: the census saga with the chosen blend. | Pin a co-op two levels up in the console: the board re-prices; the flat CO-OP Rating on the player page does not move. | The checkpoint between the halves is the one loop (§14): whether the two anchors agree well enough to blend, or the board changes shape. Slots anywhere after 3. |
...
**A loop is only something that can change the high-level plan** (owner, 2026-09-22: "only want to
loop on stuff that changes high level plans"). Everything else that needs real data or a curation pass
is a build step inside the slice that builds the tool for it, and lives in that slice's row in §12: the
formula sanity probe in 1c, the Completion top-ten read in 3, the Stamina rule through the matcher's
dry run, the Gimmick tag console and the first themes in 5, days left in the pill in 2a and on the
import surfaces in 2f. Sheet D's redo is a mock round and gates 3 (§13).

The loops:

- **Can a co-op be anchored to a level at all** (D42; the checkpoint inside slice 8)? Slice 8 opens
  with a probe that prints, for every co-op, the pass-anchored level (the co-op pass tier list per
  player count) and the score-anchored level (the scoring-difficulty saga's method, with the players'
  Doubles competitive levels as the anchor). If the two halves agree well enough to blend, the owner
  picks the blend and the census saga follows; if they do not anchor, the CO-OP PUMBILITY board changes
  shape — tier-priced, or dropped — and that is a plan change, which is why this is a loop.
- **The 20 / 20 / 50 numbers in D7** after the first balanced season, with the census in §5 re-run:
  they can change the balancing rule.
- **Season rewards**: what the top of a sealed board earns, and how many places. A product decision
  that adds a slice, taken before the first sealed board anyone sees.

Build-time decisions, taken in the slice that touches them (recommendations recorded, none ruled on):

- **Where the last days of a season are signposted** (raised by the no-grace ruling, D13): days left
  in the pill, where Sheet A already places it (2a), and one line on the import surfaces (2f). The
  alternative is a countdown in the import widget.
...
- **Whether slice 3 builds the all-time Hardmode board as a list-board from day one** — the list
  refreshed by the weekly census event, the two list tables pulled forward from slice 5 — rather than
  rendering it through the six columns and re-plumbing it in a later cleanup (D45). Recommended yes:
  the six columns would keep feeding the Hardmode page's own standing and nothing would need retiring.
  Not ruled on; slices 3 and 5 read as written until it is.
