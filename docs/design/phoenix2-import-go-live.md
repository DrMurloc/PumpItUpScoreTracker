...
| `my_page/pumbility.php` | **Per-chart PUMBILITY + grade + plate** (e.g. ALiVE D21 978,147 FG → 343.57) | Consumed 2026-07-19 by `LiveSite/PumbilityOfficialReconciliationTests` — settled the formula: singles price one level up the base curve, sub-10 charts price at zero, A = 1.28. Now also read on every import by `ScoringObservations`, whose telemetry settled the rest: both constant tables turn out to be per chart type ([phoenix2-implementation.md](phoenix2-implementation.md)) |
