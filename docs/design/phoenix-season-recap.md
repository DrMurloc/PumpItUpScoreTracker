...
The highlights slide also carries **Rarest passes** (round two, denominator fixed in round three): the player's top 5 passes by fewest sitewide passers, with the rate shown as passers ÷ **every active PIUScores player** — NOT players who recorded the chart (on a boss chart the recorders are exactly the peers who pass it, which read as "45% pass it" for a chart ~3% of the site has passed). **No record-count floor** (owner call): the low-record tail IS the boss ladder — Ultimatum/Neo Catharsis D27 have one passer sitewide, Kugutsu D27 three — and a barely-recorded S4 counting as "rare" is honest, obscurity being a legitimate flavor of rare.
