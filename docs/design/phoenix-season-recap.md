...
- **Backfilling after a logic change**: for single-section reworks, prefer a **targeted patch command** over the full sweep — `RebuildRecapPgCardsCommand` (admin button "Rebuild Recap PG Cards") rewrites only `ImpressivePgs` on every stored recap, leaving each payload's rivals/weekly/`ComputedAt` untouched. This is also why the PG rework did NOT bump the schema version: a bump invalidates every row and forces the full rebuild a targeted patch exists to avoid (old `ImpressivePgs` items deserialize into the new shape harmlessly until patched). Either sweep is side-effect-free by construction — RecapSaga has no bus publisher; Discord cards ride the import pipeline's `ScoreHighlightsCapturedEvent`, which the saga consumes but never emits.
...
### 5. Rivals
3 singles + 3 doubles rivals, **public users only**, ranked by overlap of top-50 competitive (fung) chart-id sets. Tiers are **strict priority, not thresholds** (owner call after round one — a ≥3-candidates-or-fall-through rule surfaced strangers over community mates): ANY member of your user-created communities within **±0.5** outranks everyone outside them; your country community and then the global pool (tight **±0.25**) only top up leftover slots. The community window is deliberately wider — at the top of the ladder ±0.25 is a handful of players worldwide, and a rival you know at 0.4 away beats a matched stranger. **World is excluded by name, never by flag**: production data carries World with `IsRegional = 0`, so trusting the flag folds all of World into the community tier and rivals degenerate to the global pool (found live, 2026-07-10).

### 6. Most impressive PGs
Round four (post-release feedback — the tier-list Hard-or-higher filter read badly live): your **top 50 PGs by level** form the pool; within it the **rarest 5 win** — fewest PG holders sitewide (from `ChartScoreAggregate.PgCount`), shown as "only X% have it" against the whole active population, level breaking ties. The "PG" tier list is no longer consulted.
...
The highlights slide also carries **Rarest passes** (round two, denominator fixed in round three): the player's top 5 passes by fewest sitewide passers, with the rate shown as passers ÷ **every active PIUScores player** — NOT players who recorded the chart (on a boss chart the recorders are exactly the peers who pass it, which read as "45% pass it" for a chart ~3% of the site has passed). **No record-count floor** (the low-record tail IS the boss ladder — Ultimatum/Neo Catharsis D27 have one passer sitewide), but a **level floor of your competitive level − 6** per chart type (owner call, round three: without it the list was S2s the elites never bothered recording). Co-op is floor-exempt — its "level" is a player count, and assembling four players for Gargoyle co-op IS the rarity (28 passers sitewide).
...
### 11. Phoenix 2 finale
P1 scores projected onto Phoenix 2: carried-over charts (4,367 of 4,571 — 95.5%) rescored with P2 `ChartMix` levels through the mix-keyed P2 formula (PR #128), **per-pool Singles and Doubles Pumbility totals + two projected titles** — the highest `Phoenix2PumbilityTitle` from the Singles pool ladder and the highest from the Doubles pool ladder (`PumbilityPool.Singles`/`Doubles`; the hidden Total tiers are not projected). The slide's overall *projected total* number is a single merged top-50 across both types (corrected 2026-07-13 — was the two per-type pools summed); a targeted `RebuildRecapTotalPumbilityCommand` admin button repatches just that field on stored recaps. Green accent; "see you in Phoenix 2". Note the existing `PumbilityProjectionSaga` is P1→peer-expectation, *not* this — the P1→P2 rescoring is new code.
