...
- **Backfilling after a schema/logic change**: "Compute All Season Recaps" IS the backfill, and it is side-effect-free by construction — RecapSaga has no bus publisher; it only writes recap rows. Discord cards ride the import pipeline's `ScoreHighlightsCapturedEvent`, which the recap sweep consumes but never emits.
