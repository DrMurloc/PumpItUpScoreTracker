...
| `scores.ChartMix` | Chart↔mix mapping with the level and note count for that mix, plus `LegacySlot` (pre-Exceed slot identity — "Crazy", "Another Nightmare" — part of chart identity in those eras; [legacy-mixes design](design/legacy-mixes.md)) and `AddedInVersionId` — the patch of *that mix* the chart first appeared in (nullable = unknown; a carry-over reads the mix's launch version; FK onto `MixVersion`, declared by Catalog's model contribution; [chart-versions.md](design/chart-versions.md)) |
