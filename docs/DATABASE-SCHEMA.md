...
| `scores.ChartMix` | Chart↔mix mapping with the level and note count for that mix, plus `LegacySlot` (pre-Exceed slot identity — "Crazy", "Another Nightmare" — part of chart identity in those eras; [legacy-mixes design](design/legacy-mixes.md)) and `AddedInVersionId` — the patch of *that mix* the chart first appeared in (nullable = unknown; a carry-over reads the mix's launch version; FK onto `MixVersion`, declared by Catalog's model contribution; [chart-versions.md](design/chart-versions.md)). Not touched by seasons: a season's chart rating is a `ChartSeason` row in the Catalog section ([seasons](design/seasons.md) D33) |
| `scores.Country` | Country list with flag image path |
| `scores.Avatar` | The official avatar catalog: one row per **listed entry** (412), grouped by `PictureId` into 182 distinct pictures and by `GroupId` into 170 avatars, each row carrying the mix that lists it as a `1 << (int)MixEnum` bit. Every listing is kept rather than only the deduped pictures because the table has to recognise the url a player already wears, and each mix mirrors the same picture under its own prefix. Seeded by migration from the three official avatar pages and otherwise static — the dedupe behind it is a pixel comparison, not something to re-derive at request time. Names are deliberately **not** unique: the official pages ship `Electra` twice, plus `Hero`/`hero` and `Miya`/`MIYA` ([avatar-selection.md](design/avatar-selection.md)) |
...
| Table | Purpose |
|---|---|
| `scores.PhoenixRecord` | Best-known Phoenix-scoring attempt per user+chart+mix: score, plate (NULL when broken — the game awards none for a failed stage), broken flag, and the `Source` of the current best (verified ⇔ `officialImport`; NULL predates capture). `Perfects/Greats/Goods/Bads/Misses` hold the producing play's judgement breakdown (all five set together; NULL = never observed); `MaxCombo` is the combo solved from that breakdown and the score at write time (NULL when unsolvable — no breakdown, no note count, or a breakdown short of the chart; re-derived by the *Backfill max combos* admin button). A stage break is never a row here ([stage-breaks-and-max-combo.md](design/stage-breaks-and-max-combo.md) D10). Unique on UserId+SeasonId+ChartId+MixId; pre-Phoenix-2 rows backfilled as Phoenix — as are all MixId columns below. `SeasonId` (smallint, 0 = all-time) marks a **seasonal** personal best — a second row per season beside the all-time one, hidden from every read by the `AllTime` query filter unless a reader drops it; the unique key is (UserId, SeasonId, ChartId, MixId) — [seasons](design/seasons.md) §6.2 |
...
| `scores.ScoreEventJournal` | **Append-only** journal of observed plays. One row is one play, uniquely keyed **(UserId, MixId, ChartId, OccurredAt)** — the site's own play time — so re-importing a recently-played window collapses onto the existing row instead of duplicating it. `IsBest` says whether the play became the record when it was written; rows sourced from recently-played that beat nothing are `IsBest=0`, and are the only rows the ledger's record ignores. Rows are never updated (except to raise `IsBest`) or deleted. `SessionId` groups rows into play sessions / import runs (NULL predates capture). `Perfects/Greats/Goods/Bads/Misses` hold the play's judgement breakdown when observed (all five together; NULL otherwise) and `MaxCombo` the combo solved from it (NULL when unsolvable); `IsStageBroken` marks a play the stage interrupted — always `IsBest=0`, `Score` NULL (the site's running score is not a chart score), plate NULL, breakdown when the recently-played card carried one, none when the row came from a Phoenix 2 best-list card ([stage-breaks-and-max-combo.md](design/stage-breaks-and-max-combo.md) D11); `IsNonLifebarBreak` says the life bar provably could not have emptied, so a **Stage Pass command** ended the stage, and `PassPlate`/`PassGrade` name the command's target — a target the run could have just crossed, settled across a session's replays of the chart where they agree and by evenly spread breaks where they cannot — two independent axes, either or both may be set, and all three are NULL on a row with no judgement breakdown or no catalog level. They are derived, not observed: the site prints no command on any card, and the Backfill stage break causes button re-derives them wholesale when the formula improves ([pass-command-detection.md](design/pass-command-detection.md) D29-D43); `OccurredAt` carries the official site's saved timestamp when the import had one, clock time otherwise. Seeded 2026-06 from `PhoenixRecord` (`Source='backfill'`, dated at the record's last update); every pre-2026-07-30 row is `IsBest=1`, because until then the journal was written only when the record changed. Rows *are* deleted by a player deleting their own data — the append-only rule governs the write path, not the owner of the data ([delete-my-data.md](design/delete-my-data.md) D8). `LetterGrade` is the XX-and-older axis: legacy has no plate — the letter IS the plate equivalent — and it is what most legacy records carry, since `Score` is usually absent there. It is a separate column rather than a reuse of `Plate` because a Phoenix reader parsing "SSS" as a plate throws; the row's mix decides which pair is live. Note that a legacy `Score` can exceed 1,000,000 (the largest in production is 45,282,000), so it is read as an era score rather than a `PhoenixScore`. See [score-truth-model.md](design/score-truth-model.md) and [legacy-mixes.md](design/legacy-mixes.md) |
...
| Table | Purpose |
|---|---|
| `scores.PlayerStats` | Aggregated player stats per mix (PK SeasonId+UserId+MixId — `SeasonId` 0 = all-time, a season's row is a second row behind the `AllTime` query filter, [seasons](design/seasons.md) §6.2; `TotalPumbility` is the season board's total, empty until slice 1b): ratings, competitive levels, clear counts, the three PUMBILITY pools (`SkillRating`/`SinglesRating`/`DoublesRating`) and the CO-OP Rating (`CoOpRating` — not a pool: every co-op chart summed, on both mixes since 2026-08-17; before that Phoenix 2 stored 0 here) stored as **float, unrounded** — rounding one at rest costs precision only the UI may spend, and two surfaces rounding at different points disagree about the same account (2026-08-09), plus the three **Hardmode** pool totals (`HardmodeRating`/`HardmodeSinglesRating`/`HardmodeDoublesRating`, float on the same terms, with a held count per pool beside them (`HardmodeChartsHeld`/`HardmodeSinglesChartsHeld`/`HardmodeDoublesChartsHeld` — three separate top-fifties, so forty combined and twelve doubles are both true of one player) — written by the weekly census, zero until it first runs, [hardmode-leaderboard.md](design/hardmode-leaderboard.md)), plus the `Estimated*PumbilityRank` trio and the `PumbilityBoardAsOf` snapshot they were ranked against — our pool placed into the last sealed official board, not a rank read back from it ([session-breakdown §2.2](design/session-breakdown.md)) |
...
| `scores.HardmodeChart` | The **Hardmode** chart list: the rarest slice of every Phoenix 2 folder, one row per (season, mix, chart) — `SeasonId` 0 is the census's all-time list behind the `AllTime` query filter, a season's rows a copy taken at the roll ([seasons](design/seasons.md) D31) — with the weighted hold count (`51 − slot` summed across every full pool, all three pool kinds), the distinct holder count, the folder's size and its own cut size, and `ComputedAt`. No `UserId`. Rewritten wholesale by the weekly Hardmode census and read through `IHardmodeChartReader` ([hardmode-leaderboard.md](design/hardmode-leaderboard.md)) |
...
| Table | Purpose |
|---|---|
| `scores.ChartSeason` | A chart's **season rating** where it differs from the printed level: one row per (season, mix, chart), season first, with `Level`, `PrintedLevel` (the printed level at the roll), `MovedThisRoll`, the hold behind the move (`HoldWeight`, `HoldRank`, `LivePlayers`) and `IsPinned`. Sparse and carried forward while it differs; written by the season roll and the admin pin, read as an overlay on `ChartMix.Level` in seasonal view. No user key ([seasons](design/seasons.md) D33, §6.3) |
...
| `scores.MixVersion` | One row per game patch of a mix: `Name` (`1.01.0`, bare), `ReleaseDate` (the Korean notice date; NULL on a legacy patch nobody dated), `SortOrder` (the ordering truth — by/after-version filters compare on it), unique on (MixId, Name). Seeded by migration with 128 rows (every dated Phoenix-era patch, pumpout's legacy patch names, one `Release` row per single-version mix, a `JE` pseudo-version under Prime); new patches come from the admin BulkAddCharts picker, never migrations ([chart-versions.md](design/chart-versions.md)) |
| `scores.SongMix` | One row per song per mix it has charts in, carrying the song's **channel** on that mix (`Channel`: `Original` · `KPop` · `WorldMusic` · `JMusic` · `Xross`, the enum names, required). Composite key (SongId, MixId), index (MixId, Channel); no row = unknown. Not seeded by migration: the 4,926 day-one rows are a what-if script the owner runs, and every path that gives a song a mix — the bulk-add batch, the dev harness, a hand-seeded returning song — writes the row ([song-channels.md](design/song-channels.md)) |
...
| `scores.OfficialHardmodeRating` | The Official Boards side of the Hardmode leaderboard: one row per (`OfficialPlayer.Id`, mix) carrying the three Hardmode pool totals and, per pool, how many qualifying charts it holds (`ChartsHeld`/`SinglesChartsHeld`/`DoublesChartsHeld`), written after the weekly census. Keyed by the board player, **not** a site `UserId` — the site half lives on `PlayerStats` ([hardmode-leaderboard.md](design/hardmode-leaderboard.md)) |
...
| `scores.CommunityDiscordServer` | The one Discord server a community hands out roles in: `CommunityId` (**unique** — one server per community is a constraint, not a convention), `GuildId`, `GuildName` (denormalized for the page, refreshed whenever the bot can see the server), `DesignatedAt`. Deliberately **not** the same thing as a registered feed channel — a server that receives a community's score cards has not thereby been claimed by it. Set via `/piu link-server` ([discord-role-management](design/discord-role-management.md) D3) |
| `scores.CommunityTitleRole` | One Phoenix 2 title mapped to one Discord role: `CommunityId`, `TitleName` (the shipped title name verbatim), `RoleId`. Unique both ways — a title cannot point at two roles, and two titles cannot point at one, which would leave "should this role come off" with no single answer. This table **is** the managed set: a mapped role handed out by hand is taken back ([discord-role-management](design/discord-role-management.md) D12) |
| `scores.CommunityDiscordGrant` | Who a community has granted roles to, and against which Discord account: `CommunityId`, `UserId`, `DiscordUserId`, `LastReconciledAt`. One row per (community, member), not per role. ⚠ It is what makes removal possible at all: Identity's purge drops `ExternalLogin` on its FIRST pass right after an in-memory publish, so reading the snowflake in a purge consumer races a delete it loses — and a sweep over current members never visits somebody who left. Both reads come from here |
| `scores.CommunityDiscordOptOut` | A player who turned one community's title roles off for themselves: `CommunityId`, `UserId`, `OptedOutAt`. Unique on (CommunityId, UserId). Read by the reconcile as its fifth fact, so no angle — join, sweep, a title earned later, an admin's Check now — can hand the role back; written and deleted only by `/piu roles off` / `/piu roles on`, run in the community's designated server. Its own table because a grant row is deleted the moment a member holds nothing, which is exactly what opting out makes true ([discord-role-management](design/discord-role-management.md) D22, D23) |
...
| Table | Purpose |
|---|---|
| `scores.ChartComment` | One comment, reply or personal note on a chart. `Audience` is the enum **name** (Public/Private/Community) plus `CommunityId` — keyed by id rather than by community name, so a club that renames does not strand its threads. `ParentCommentId` is null on a root and never points at another reply: threads are one level deep. `Text` is **plain text**, ≤500 characters, normalized on the way in (one newline convention, no trailing spaces, no more than one blank line in a row) so the cap counts what is stored and rendered. `SourceLanguage` is stamped by the translation pivot's detection (Slice 4) and deliberately **never guessed** — stamping the poster's UI culture would record a Korean speaker browsing in English as `en-US` and get their comment rewritten Korean-to-Korean; an edit clears it, because the detection belonged to the old words. `TranslationQueuedAt` is the edit-requeue cooldown's clock. `AnchorAt` (decimal(9,3), null) is the **second of the chart** a comment points at ([step-chart-comments](design/step-chart-comments/README.md)) — the rail pins' own clock, stored as the client sent it after snapping to the nearest arrow row; null on every reply (a reply reads its root's) and on comments about the whole chart. `DeletedAt`/`DeletedByUserId` are a soft delete; `DeletedByUserId` is the **moderator** rather than the author, which is the second user key on the row. ⚠ **Not in a `UserOwned` purge manifest**: a blanket delete by `UserId` would take a root out from under its replies, so ChartComments purges this table by hand — a root somebody replied to is tombstoned (`UserId` cleared to `Guid.Empty`, `Text` cleared) and everything else goes outright |
...
| `scores.ChartCommentReport` | One player's report of one comment, with a **resolution slot per queue** (`CommunityResolvedAt/By`, `SiteResolvedAt/By`) — a community admin's dismissal clears their panel and only theirs, an escalated report stays on the site admin's desk, and removal stamps every open slot. `Reason` is the closed enum name; `RenderingLocale` is what the reporter was reading, null until translation lands. Purge key is the **reporter** — an open report vanishing with its reporter is accepted |
| `scores.ChartCommentRestriction` | A community mute: you stay in the club and lose the mic (blocks post, reply and edit there; delete and votes untouched). Community scope only — the site scope is `User.IsContentLocked`. Lifted rows are retained (`LiftedAt`), so reads filter to active. Purge key is the muted player; `RestrictedByUserId` is the moderator and may outlive their account, like `DeletedByUserId`. **Unique over active rows** — `(UserId, CommunityId) WHERE LiftedAt IS NULL` — so two racing moderators land on one mute |
| `scores.ChartCommentArchive` | A community comment whose club was deleted: on `CommunityDeletedEvent`, ChartComments moves the club's comments here (every `ChartComment` column, plus the club's `CommunityName` snapshot — the only place the name still exists — and `ArchivedAt`; `AnchorAt` rides along, so a revived club's comments keep their seconds) and purges their votes, revisions, reports and the club's mutes. Nothing renders these rows; they exist so a revival starts from real data, matching the never-drop-tables standard. Purge key is the author — words surviving a club's death do not survive the author's deletion |
| `scores.ChartCommentRendering` | One locale's rendering of one comment, unique per (comment, locale), stored **after link substitution** — real URLs, parsed at render like any body by the same parser that judged the link-set check. `TranslatedBy` is provenance (models + path). Derived data with no user key: purge, archive and hard delete all cascade here by `CommentId`, and an edit clears the set — an edited comment must never show a translation of its previous self. Exempt from the `UserOwned` manifest with the same reason as revisions |

## Translations (vertical: `ScoreTracker.Translations`)

| Table | Notes |
|---|---|
| `scores.TranslationRequest` | One queued community text, keyed by an **opaque** `SourceKey` (unique — a re-queue replaces the row; the pipeline keeps no history). `Text` is stored **with link markers already in place**, so no URL ever sits here or reaches a model. `State` walks Pending → PivotSubmitted → PivotDone → FanOutSubmitted → Translated, with Failed off either collect (announced via `TextTranslationFailedEvent`, and re-queueable from the admin page's Retry failed). `LastSubmittedAt` **survives the upsert** — it is the submit-side cooldown's clock, making "a text enters a batch at most once per 24 h" true however often its author edits. `PivotJson` is stage one's stored output — never displayed; it is what makes a bad rendering a stage-two re-run and a sixth locale a backfill. **No user key by design**: whose text this is stays the caller's knowledge, and the caller discards rows here (ChartComments does on delete, purge and archive) when originals stop existing |
| `scores.TranslationBatch` | One submitted Claude batch — the poll list while open (`CompletedAt` null), the spend ledger once complete: the four token counts plus `CostUsd` priced at completion, so the rolling 30-day ceiling reads recorded fact rather than re-deriving from prices that may have changed since |

## Seasons (vertical: `ScoreTracker.Seasons`)
