...
| play / run (one attempt) | `run` (EN; feminine, `una run`) | intake draft accepted |
...
| Play Count / X Plays | `número de runs` / `{0} runs` | attempt = `run` per intake |
...
1. **Opening `¡` omitted on all exclamations** (`Guardado!`, `Copiado al portapapeles!`, `Bienvenido/a a Score Tracker, {0}!`). Extrapolated from the intake §4 anchors, which used `¿…?` but never `¡`. If that was an oversight rather than a style choice, sweep the file to add `¡…!` pairs. **This may be the volunteer's known open question — confirm with her.** Questions keep `¿…?` either way.
...
- The filled form arrived as a markdown export; any Word/Google-Docs comments did not survive conversion. If she flagged an open question in a comment, it needs to be re-asked directly.
