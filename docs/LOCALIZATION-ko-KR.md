...
- **`스코어` is drifting back in (29 values as of the 2026-07-26 QC pass).** The glossary rule is `Score` → 점수 and the file is 189-to-29 in its favour, but new batches keep reaching for the Konglish. The four newest offenders were swept in that pass (`Scoring level from/to` → 최소/최대 점수 레벨, `{0} scores` → 점수 {0}개, `Score count` → 점수 개수); the remaining 25 are older strings and want **one dedicated sweep**, not piecemeal edits. Note `Scoring level` (the score-derived difficulty) is 점수 레벨 — don't reintroduce 스코어링 레벨 for its range variants.
- **`Rank` → 랭킹 now renders where it never did.** The key had been shadowed by a case-variant sibling (`rank`) and dropped from every satellite; the 2026-07-26 collision fix made it live. 랭킹 is the `Rankings` loanword — for a bare column header 순위 (or 등수, per the `Place` row) reads more natural. Worth a native look now that it is actually on screen. The inline sense moved to its own placeholder key, `rank #{0}` → {0}위.
