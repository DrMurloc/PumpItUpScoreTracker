...
- **Phoenix 2**: the compute-trigger messages (tier lists, scoring difficulty, weekly rotation, leaderboard import) carry a `MixEnum` defaulting to Phoenix, and every registration above deliberately runs **Phoenix-only** for now. Phoenix 2 tier-list/stats jobs get scheduled once real score volume exists; the leaderboard import stays Phoenix-only until the Phoenix 2 mirror is built (the P2 site replaced per-level rating boards with a single login-gated Pumbility board).
