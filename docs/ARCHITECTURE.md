...
| `Competition/` | Stamina tournament flows (March of Murlocs), qualifiers submission, `/WeeklyCharts`. The bracket subsystem and the `/Tournaments` directory were deleted ([deletions-wave-1](design/deletions-wave-1.md)) |
...
| `Tools/` | calculators — `/PhoenixCalculator` (judgments → score → grade → PUMBILITY as one chain, two routes in; `/RatingCalculator` 301s here, [phoenix-calculator-redesign.md](design/phoenix-calculator-redesign.md)) and `/LifeCalculator` — plus `/ChartRandomizer` and `/MixChanges` (+ `/MixChanges/{from}/{to}` — the mix diff; `/ChartCompare` 301s here) |
