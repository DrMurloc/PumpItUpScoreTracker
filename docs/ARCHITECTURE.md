...
| `Progress/` | `/Pumbility`, `/Titles` (the **ladder rails** — one rail per title ladder rather than one row per title, see [titles-overhaul.md](design/titles-overhaul.md)), `/Player/{id}/Sessions` (the **session breakdown** — the most recent session rendered full-page, older ones a table with a View button; the Discord score card's link target, see [session-breakdown.md](design/session-breakdown.md)), `/Player/{id}/PhoenixRecap` (the season-recap slide deck, admin-computed — see [docs/design/phoenix-season-recap.md](design/phoenix-season-recap.md)). The old Player Stats pages (`/Progress`, `/Phoenix/Progress`) are **retired** — their graphs are home-page widgets now ([HomePageWidgets/by-level-breakdown.md](design/HomePageWidgets/by-level-breakdown.md)) |
...
| `CommunityTools/` | `/CommunityTools` (the directory players browse and connect from), `/CommunityTools/Invite/{code}` (a private tool's landing page), `/Developers` (the maker's console — settings, then keys, invite links and webhooks as their own sections, then Help, then the delete), `/Developers/{id}/Console` (activity log) and `/Developers/{id}/Debug` (test delivery, replay) — [api-v2-community-tools.md](design/api-v2-community-tools.md) |
