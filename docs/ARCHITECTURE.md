...
| `Progress/` | `/Progress`, `/Phoenix/Progress`, `/Pumbility`, `/Titles` (the **ladder rails** — one rail per title ladder rather than one row per title, see [titles-overhaul.md](design/titles-overhaul.md)), `/CompetitiveLevel`, `/Player/{id}/Sessions` (public session roundups + score journal — the Discord score card's link target), `/Player/{id}/PhoenixRecap` (the season-recap slide deck, admin-computed — see [docs/design/phoenix-season-recap.md](design/phoenix-season-recap.md)) |
