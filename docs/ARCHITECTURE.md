...
| `Progress/` | `/Pumbility`, `/Titles`, `/CompetitiveLevel`, `/Player/{id}/Sessions` (public session roundups + score journal — the Discord score card's link target), `/Player/{id}/PhoenixRecap` (the season-recap slide deck, admin-computed — see [docs/design/phoenix-season-recap.md](design/phoenix-season-recap.md)). The old Player Stats pages (`/Progress`, `/Phoenix/Progress`) are **retired** — their graphs are home-page widgets now ([HomePageWidgets/by-level-breakdown.md](design/HomePageWidgets/by-level-breakdown.md)) |
