...
**A similarity band is derived from the scale it measures, never fixed.** Where a control says
two things are "worth about the same", the tolerance comes from the spread of the values it is
pointed at. Phoenix PUMBILITY spans 75× and Phoenix 2 barely 2×, so the calculator's old flat
±10% highlighted 7% of one grid and 44% of the other; half of one folder step off the mix's own
base curve lands on ~5% in both and re-tunes itself if a curve is re-derived. Where the user
wants a shortlist rather than a landscape, bound it by something the domain understands —
folders away — rather than by any tolerance at all
([phoenix-calculator-redesign.md](design/phoenix-calculator-redesign.md) §3.3, §3.5).
...
**4. No raw color literals.** ⚙ *Ratcheted by `UiColorTokenTests`.* UI code under `Pages/`, `Components/`, and `Shared/` reads theme tokens, not hex strings, `Colors.*` constants, or functional notations (`rgb()`, `rgba()`, `hsl()`) — that last form was the ratchet's blind spot until the calculator overhaul, which is how the rating calculator's hand-rolled rgb lerp lived there unlisted. The allowlist is launch-day debt and only shrinks; each page overhaul burns down its own entries. (Exception by design: the Phoenix Recap deck is self-styled slide art and stays allowlisted.)
...
- **`UiColorTokenTests`** (ArchitectureTests) scans `Pages/`, `Components/`, `Shared/` for hex literals, `Colors.*` constants and functional notations (`rgb()`/`rgba()`/`hsl()`) against a shrink-only allowlist. Exceeding an allowance fails; dropping *below* one also fails until you lower the entry — that's the ratchet. It reads `.razor` and `.cs` only, so pasted `.cshtml` documents (the privacy notice) are out of range by design.
