...
**3. One concept, one component.** Difficulty is always `DifficultyBubble`; a grade is always `LetterGradeIcon` (or `GradeBandIcon` where the value is a world-first band string and the top rung is the perfect-game plate); a score is always `ScoreBreakdown`; a player is always `UserLabel`. A new visual concept means a new shared component in `Components/` — never a page-local restyle of an existing one.
   ...
   **Identity is paint before it is furniture.** The same label carries the country as a *faded underlay* behind the avatar and the name (`--label-flag`, masked so it fades out before the row's figures), never as a picture in front of them. A 15px flag and its gap cost ~26px of the name's room, and on a phone that is most of it: the community rankings' Player column resolves to ~96px there, so a flagged player printed four letters and an ellipsis. Country flags are also a *weak* identifier at 15px — half the world's are three coloured bars — so the country's **name** rides the label's tooltip beside the game tag, where it can actually be read, and the wash carries recognition rather than information. This is the general rule for any attribute that decorates an identity rather than distinguishing it: paint it, don't seat it in the row.
