...
**5. Density is a setting, not a redesign.** Three sanctioned densities — **Comfortable** (cards), **Compact** (the jacket "sticker sheet"), **Table** (sortable rows) — stored per user in UiSettings under `Universal__Density`. A collection page picks its default and honors the override; it never invents a fourth mode. (Ships with the tier-list overhaul; the key is reserved now.)

**6. Filters are furniture.** One collapsed, sticky filter row; active filters render as removable chips; the full panel lives in a drawer. Filters never push the answer below the fold.

**7. Design for +40% text.** Eight-plus locales; Portuguese and French run long, CJK runs dense. Every string goes through `L[…]`, new keys land in **every** locale in the same pass (glossaries: `LOCALIZATION-<locale>.md`), no fixed-width labels, no truncation without a tooltip.
