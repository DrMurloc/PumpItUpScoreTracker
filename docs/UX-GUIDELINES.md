...
**Your own scores are colored by your standing among your peers — and the player chooses both the peers and the ramp** ([peers-abstraction.md](design/peers-abstraction.md)). The `/Account` dialog picks the peer sources (rivals, competitive level, PUMBILITY peers, any community incl. region and World — a union), one of nine color systems (the rarity ramp above is the default; classic, letter-grade metals, podium, single hue, result screen, three steps, actual letter grade, none) and one glow rule. **Glow is a threshold signal, not a spectrum** (owner, 2026-09-05: *"the color is the spectrum"*): one rule — Perfect Games only · Top N places · Top N% · Off — and one strength (`.rarity-glow-2`) for whatever it lights; Off switches off the Perfect Game glow too. The `.rarity-glow-1..3` ramp survives for the surfaces that never meant standing (title rungs, folder chips, the widget's headline number). Every such score renders through `PeerScore`, which tap-opens the standing popover — never a hover tooltip, because a phone has no hover.
