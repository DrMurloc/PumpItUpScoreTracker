...
**3. One concept, one component.** Difficulty is always `DifficultyBubble`; a grade is always `LetterGradeIcon` (or `GradeBandIcon` where the value is a world-first band string and the top rung is the perfect-game plate); a score is always `ScoreBreakdown`; a player is always `UserLabel`. A new visual concept means a new shared component in `Components/` — never a page-local restyle of an existing one.
   ...
   **The avatar belongs to the label, at the caller's size.** Boards that drew their own avatar beside the label hand it over (`ShowAvatar`), because a wash under both cannot be owned by the component if one of them lives outside it. Size and shape stay per-caller (`AvatarSize`, `SquareAvatar`) — the challenge boards run 20px circles, the widgets 24px, the community rankings a 26px square — and unifying them is a separate, deliberate pass. Rows the site cannot name (your own "You" row, a deleted account, an Official-scope board tag) have no label to draw a face, so they keep a standalone avatar.
