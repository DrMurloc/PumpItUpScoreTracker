...
2. **Semantic tokens** — colors that carry *data meaning*. Hue is tuned per mix; **meaning and ordering never change**:
   ...
   - `--judg-*` — the game's own **judgment** vocabulary: perfect ice-blue, great green, good amber, bad violet, miss red. Reached via `ThemeScales.JudgmentColor(judgment)`, or `MixThemes.JudgmentHex` for render targets that can't read custom properties (ApexCharts). **Mix-invariant, like the alert colors** — a MISS must read as a miss in every theme. Judgement **counts** are the one score fact that renders as coloured *text*, not art — the shared `JudgementStrip` (colour is the label, the count is the value), matching the game's own result screen; grades and plates stay images per rule 3.
   ...
   - `--ptype-1`…`--ptype-5` — the five **playstyle archetypes** (Pass Pusher → Perfectionist) in the chip's own grade-metal hues: FG copper, MG silver, SG gold, UG ice, PG pale ice, the summit one glowing the way `PlayerTypeChip` glows it. Mix-invariant, and deliberately its own group rather than a reuse of `--plate-*` — an archetype is not a plate, and a plate re-colour must not move it. `MixThemes.PlayerTypeHex(type)` is the accessor for render targets that cannot read custom properties ([design/pumbility-overhaul.md D69](design/pumbility-overhaul.md)).
   - `--classic-1`…`--classic-7` — the **classic** score ladder, the retired Raider.io-style ramp with its hues retuned for the dark themes (grey → white → green → blue → purple → orange → pink at the top 1%). Mix-invariant. One of the nine score color systems a player can choose (below); `ThemeScales.ScoreStyle` is its only consumer.
   - `--hue-1`…`--hue-6` — **single hue**, six lightness steps of the mix's primary, dark to bright. Per mix. Ordered by lightness alone, so it reads the same for every kind of color vision; the accessibility answer among the score color systems.
   - `--hard-mark` — the **Hardmode** mark: the red that says a chart is in the week's Hardmode list, or that a number belongs to the Hardmode pool rather than the PUMBILITY one. Mix-invariant, like `--judg-*` and `--life-*` — a Hardmode chart is a Hardmode chart in every theme. One token rather than a ramp, because it encodes membership, not degree: it paints the score badge, the gain chip, the milestone strip's rail, the session title-rail's gain segment, the feed captions, and the difficulty-bubble glow, so the same fact wears the same red everywhere. `ThemeScales.HardmodeMark` is the accessor. The glow itself is a `drop-shadow` filter rather than a border or a fill, so it haloes the bubble art's own alpha instead of boxing it ([hardmode-leaderboard.md D19](design/hardmode-leaderboard.md)).
   - `--easy-mark` — the **difficulty glow's** green end: the mint that says a chart is one of the most-held in its folder, the opposite end of the same weekly count the Hardmode red comes from. Mix-invariant, one token, membership not degree — the same reasons as `--hard-mark`, and drawn with the same three stacked `drop-shadow`s (`.bubble-easy` beside `.bubble-hardmode`) so the two ends read as one family. **Mint, never the theme's green**: the glow only renders on Phoenix 2, whose primary, links and emerald rarity step are already green, and a halo in the same hue reads as theme chrome rather than as a fact. `ThemeScales.EasyMark` is the accessor. The glow is its own account setting, **Show difficulty glow**, on by default ([hardmode-leaderboard.md D32](design/hardmode-leaderboard.md)).
   - `--vary-1`…`--vary-5` — **variability**, how far apart a peer group's scores sit on one chart, from *Very consistent* (cool cyan) through *Mixed* (the muted grey) to *Very split* (hot). Five levels, mix-invariant, `ThemeScales.VariabilityColor(level)`. It is a scale of disagreement, not of quality, which is why it is neither the rarity ramp nor the difficulty ramp — and **the word always prints beside the colour**: a Compact tile's dot carries the level name in its tooltip, a card body's meter is followed by the word. Introduced with the PUMBILITY peers page ([pumbility-overhaul.md §3.10](design/pumbility-overhaul.md)).
...
Percentile semantics are the established `ScoreRankingRecord.Ranking` convention: fraction of the comparable population at or below you, 1.0 = first place. Because gold is inherently brighter than sapphire, hue luminance can't order the bands — the **printed standing** always renders alongside the color (`#6 of 94 peers`), and that is the ordering channel.

**Your own scores are colored by your standing among your peers — and the player chooses both the peers and the ramp** ([peers-abstraction.md](design/peers-abstraction.md)). The `/Account` dialog picks the peer sources (rivals, competitive level, PUMBILITY peers, any community incl. region and World — a union), one of nine color systems (the rarity ramp above is the default; classic, letter-grade metals, podium, single hue, result screen, three steps, actual letter grade, none) and one glow rule. **Glow is a threshold signal** (owner, 2026-09-05: *"the color is the spectrum"*): one rule — Perfect Games only · Top N places · Top N% · under N points from the next grade · the last N% of a grade · Off — and one strength (`.rarity-glow-2`) for whatever it lights, with one exception: the two grade rules, which measure a score against its next grade rather than its peers, may instead **brighten the closer the score gets** — faint where the window opens, full at the line ([peers-abstraction.md](design/peers-abstraction.md) D39–D43). Off switches off the Perfect Game glow too. The `.rarity-glow-1..3` ramp survives for the surfaces that never meant standing (title rungs, folder chips, the widget's headline number). Every such score renders through `PeerScore`, which tap-opens the standing popover — never a hover tooltip, because a phone has no hover.
...
**A card's border says which claim it carries, and gold means "this one counts".** **To-Do outranks every other border** (owner, 2026-09-12: it "overwrites other boarder colors") — every other state reports something the record already knows, while To-Do is the one the player put there, so a pass or a custom state that could hide it would break the flag. Below it the language is owner-locked — solid success = passed, dashed info = To-Do, dashed success = passed in another mix, dotted grey = played and not passed — and Hardmode adds the one state a PUMBILITY surface needs: **solid `--rarity-gold` = in your top 50**, taken from the share card's own "Top 50 — combined" boundary so the same fact wears the same colour in the page and in the download. Gold outranks green where both apply, matching the card composer's precedence (Top 50 before Pass). On `/Pumbility/Hardmode` that makes the three states read at a glance: gold counts, green you cleared but it does not count here, plain is the chart you have not played ([hardmode-leaderboard.md](design/hardmode-leaderboard.md) D12).
...
**3. One concept, one component.** Difficulty is always `DifficultyBubble`; a grade is always `LetterGradeIcon` (or `GradeBandIcon` where the value is a world-first band string and the top rung is the perfect-game plate); a score is always `ScoreBreakdown`; a player is always `UserLabel`; where your counts sit in a cohort's spread, level by level, is always `LevelSpreadChart`. A new visual concept means a new shared component in `Components/` — never a page-local restyle of an existing one.
   **A component owns its own narrow-width behavior.** `UserLabel` is the worked example: it renders its own `.user-label` flex row (avatar pinned, name the only part allowed to shrink, `max-width:100%`) so *every* bounded caller truncates the name. Left to callers, each one re-derived it and the ones that didn't printed a bare "…" for anyone with a country flag — the label overflowed as a single atomic box and the container's ellipsis swallowed it whole. A caller's job is to bound the width; the component's job is to survive it.
   **Identity is paint before it is furniture.** The same label carries the country as a *faded underlay* behind the **name** (`--label-flag`), never as a picture in front of it and never under the avatar — a face is a picture already, and a flag washed under one reads as a printing error rather than as a country. **A flag keeps its own aspect, at one size for every name** (`aspect-ratio: 11 / 7` with `background-size: contain`, never `cover`): the underlay's box takes its height from the row and its width from the flag's own shape — every country PNG is 22×14 — so the flag sits at the head of the name at row height whether the name is two letters or twenty, and a long name simply has bare board after it. When the box was the name's width instead, both sizing modes failed at an end: `cover` cropped a flag to a blown-up horizontal slice (Germany a red bar, Japan a white one), and `contain` shrank it below row height the moment a name was short, so the country ended early for exactly the players with the shortest ones. A 15px flag and its gap cost ~26px of the name's room, and on a phone that is most of it: the community rankings' Player column resolves to ~96px there, so a flagged player printed four letters and an ellipsis. Country flags are also a *weak* identifier at 15px — half the world's are three coloured bars — so the country's **name** rides the label's tooltip beside the game tag, where it can actually be read, and the wash carries recognition rather than information. This is the general rule for any attribute that decorates an identity rather than distinguishing it: paint it, don't seat it in the row.
   **The avatar belongs to the label, at the caller's size.** Boards that drew their own avatar beside the label hand it over (`ShowAvatar`), because the wash cannot be owned by the component while the thing it has to clear lives outside it. **Every avatar is a circle** — shape is the component's, not the caller's; the community rankings' square was the last holdout and one board wearing a different face shape read as a bug rather than a variation. Only the size is per-caller (`AvatarSize`): 20px on the challenge boards, 24 on the widgets, 26 on the community rankings. Rows the site cannot name (your own "You" row, a deleted account, an Official-scope board tag) have no label to draw a face, so they keep a standalone avatar.
   **A score is the stacked grade-and-plate, everywhere a row or card prints one** (owner, 2026-09-05: "everywhere we can … will make song names show a bit more on mobile"). `ScoreBreakdown Stacked="true"` renders the `.sbd-score-stack` grid — grade over plate at 11px in one column, the number beside them spanning both rows — so the pair is exactly as tall as the score and the song name keeps the width. The Player page's head-to-head and the chart boards were the idiom; the session rows, the MoM boards and cards now wear it too, and `PeerScore` carries it through as `Stacked`. Side-by-side icons at 20px are the exception a wide, uncrowded surface has to argue for, not the default.
   **The difficulty bubble comes before the song image, always** (owner, same day: "I think we get that wrong in a few places"). A row reads bubble → jacket → name, in that order: the bubble is the fastest thing to recognise at a glance, the jacket the second, the name the fallback. `SessionScoreRow` had the jacket first and was corrected; a new row that puts the jacket first is a bug, not a variation.
   **The score straddles the jacket's bottom edge, always — and the song name rides above it.** This is the default for every chart card
   (owner, 2026-09-12: *"That's the default we are using for chart cards"*), so it is `TierListChartCard`'s job and not a per-page decision: the
   PUMBILITY pages, the tier lists, the session pages and the chart dialog all get it from the one component. The score (or the letter grade and
   plate, or a legacy grade) is the card's anchor, centred on the divider so it reads as belonging to both the art and the card; a song name goes
   **above** it, on the art, rather than displacing it. Measured, because it looked right and was not: with a name shown the score sat **5.6px below**
   the edge and with names off **1.6px above** it, and the named card was 7.2px taller, so a folder of cards had neither its numbers nor its bottoms
   in line. The cause is worth knowing, because the obvious markup reproduces it: the head is a bottom-justified flex column pulled up 30px onto the
   jacket, so a name added a line to a box whose TOP was pinned — the box grew downward and carried the score with it. The name is therefore taken
   out of the head's flow (`position: absolute; bottom: 100%`) **when there is a score**, which makes the head's height, and so the score's position,
   independent of whether a name is shown. With no score the name stays in flow and takes the edge itself, there being nothing else to anchor. The
   jacket's scrim reaches its midpoint at 62% rather than sliding straight from 55% to the bottom, because the name now lands higher on the art than
   it used to. Two positions follow from "the score is the anchor", and both were field-test corrections (owner, 2026-09-12: *"song name and pumbility
   chip too high"*): the name hangs off the SCORE's top edge, not the head's — against the head it floated 14px clear with a band of bare artwork
   between the two lines — and **a printed corner value sits ON the score's line**, at its end, rather than taking a line of its own above the
   artwork. It falls back to the name's line only when there is no score for it to sit beside.

   **A chart card's song name stops where its printed value starts.** `TierListChartCard` prints the name at 14px, and in Comfortable a printed corner value — the PUMBILITY gain on Play, what a chart is worth on Breakdown — takes the end of the name's own line, with the name the only part of that line allowed to shrink. A long name therefore ends in "…" right before the value. Parked on the jacket's corner instead, the value covered the end of the name line, "…" included, so names read as running underneath it (owner, 2026-09-11: 14px, and the value stays bottom-right even though about a third of names get cut beside a pool value). Two layout traps kill a "…" silently, and both had shipped: `text-overflow` on a `display:flex` element never draws — the text clips mid-letter and a `flex:none` sibling after it is pushed out of view (the similar-charts card lost its link icon), so the text needs its own element with `min-width:0`; and a `MudTooltip` root is `inline-block`, so inside a block parent it grows to the whole name and the ellipsis on its child never engages (the randomizer tile) — a flex column stretches it, anywhere else it needs `display:block` from a parent-scoped selector, because the root's own classes win at equal specificity. A truncated name still owes the full name a tooltip (rule 7).

   **A card says "clickable" with a ring, not by lighting its artwork.** Hover puts a `--mix-primary` ring outside the card — the treatment
   `.sbd-card` uses on the session pages, which is where the owner pointed (2026-09-12) — and deliberately does not touch the border, because that
   border is owner-locked state language (dashed info = To-Do, solid success = pass, dashed success = another mix, solid gold = your top 50) and a
   hover that recoloured it would overwrite what the card is telling you. Only the two regions that actually open the chart arm the ring, the jacket
   and the head; the body's chips and captions do not, so hovering them promises nothing. Brightening the jacket was the old treatment and went with
   the play button that used to cover it: a full-bleed control that autoplayed a video when you clicked the art, which is not what a player clicking
   a song jacket is asking for.
...
**Comfortable is two cards per row at phone width.** The grid is `auto-fill` against a 170px minimum, which needs ~350px of content box for a second column — a narrow handset has less once the page's padding is out, so the tiles went full width and a fold showed two charts. Below 480px the column count is stated (`repeat(2, minmax(0, 1fr))`) rather than derived. Compact is unaffected; its 96px minimum fits three across on the same screen. (Owner, PUMBILITY-targets field test round three: *"there's thin mobile phones that are showing 1 per row right now"* — it landed on both the tier lists and the PUMBILITY page, which share the grid.)
...
**7. Design for +40% text.** Eight-plus locales; Portuguese and French run long, CJK runs dense. Every string goes through `L[…]`, new keys land in **every** locale in the same pass (glossaries: `LOCALIZATION-<locale>.md`), no fixed-width labels, no truncation without a tooltip. **Universal terms never translate**: in-game memes ("Why Don't You Get Up and Dance, Man?") and community proper nouns (Chabala, PIU Center, PG) keep their original value in every locale — as does the piucenter **badge vocabulary** ("Anchor Runs", "Twist 90"), which routes through `L[…]` so translating it later is a resx change, but ships English everywhere ([chart-identity.md](design/chart-identity.md)).

**8. Color is never the only channel.** Every color encoding pairs with a second signal: a colored score's printed standing (a place where one exists, a percentile otherwise), pass/fail borders + icons, S/D bubbles + the S/D numeral. Verify new encodings under a colorblind simulator before shipping.
