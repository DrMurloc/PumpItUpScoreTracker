...
**3. One concept, one component.** Difficulty is always `DifficultyBubble`; a grade is always `LetterGradeIcon` (or `GradeBandIcon` where the value is a world-first band string and the top rung is the perfect-game plate); a score is always `ScoreBreakdown`; a player is always `UserLabel`. A new visual concept means a new shared component in `Components/` — never a page-local restyle of an existing one.
   ...
   **Identity is paint before it is furniture.** The same label carries the country as a *faded underlay* behind the **name** (`--label-flag`), never as a picture in front of it and never under the avatar — a face is a picture already, and a flag washed under one reads as a printing error rather than as a country. **A flag keeps its own aspect** (`background-size: contain`, never `cover`): the underlay's box is as wide as the name, so covering it would crop a 3:2 flag down to a blown-up horizontal slice — Germany a red bar, Japan a white one. It sits at the head of the name at whatever size the row's height allows, and a long name simply has bare board after it. A 15px flag and its gap cost ~26px of the name's room, and on a phone that is most of it: the community rankings' Player column resolves to ~96px there, so a flagged player printed four letters and an ellipsis. Country flags are also a *weak* identifier at 15px — half the world's are three coloured bars — so the country's **name** rides the label's tooltip beside the game tag, where it can actually be read, and the wash carries recognition rather than information. This is the general rule for any attribute that decorates an identity rather than distinguishing it: paint it, don't seat it in the row.
