...
- **Localization** — `AddLocalization` with `Resources/` path; supported cultures `en-US, pt-BR, ko-KR, en-ZW, es-MX, fr-FR`; default `en-US`.
...
- **Mix** — A version of Pump It Up (e.g. Phoenix, XX). Modeled by `MixEnum`.
- **Chart** — A specific difficulty/type playable variant of a `Song` (e.g. S20 single-player level 20). Modeled by `Chart`.
- **Phoenix score / XX score** — Scoring schemes for the two main mix families. Separate value types: `PhoenixScore`, `XXScore`.
- **Letter grade / Plate** — Performance ratings on a chart attempt. `PhoenixLetterGrade`, `XXLetterGrade`, `PhoenixPlate`.
- **Pumbility** — A composite player rating computed from top scores; calculated by `PlayerRatingSaga` / `WorldRankingService`.
- **Tier list** — A categorization of charts by difficulty/skill bucket. Generated by `TierListSaga`.
- **Bounty** — A per-chart score-target reward; tracked by `BountySaga`/`IChartBountyRepository`.
- **UCS** — User-Created Step (community-authored chart). See `IUcsRepository`, `UcsSaga`.
- **Saga (in this codebase)** — A class grouping one MassTransit consumer with related MediatR handlers for a single feature. **Not** a MassTransit `MassTransitStateMachine` saga.
