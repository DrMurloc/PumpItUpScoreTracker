...
- **Exploration workbench (manual only, NOT CI, NOT feature testing)**: `dotnet test ScoreTracker/ScoreTracker.ExplorationTests/ScoreTracker.ExplorationTests.csproj` — a sanctioned place to crawl live pages (read-only unless the owner explicitly asks for a mutation), download files, and try formulas against real data/images (OCR experiments land here). Holds the PIU live-site probes (`LiveSite/`, `[LiveSiteFact]`) and the Discord card canary (`DiscordCanary/`, `[DiscordCanaryFact]`), both moved out of `Tests.Integration`. Every test is config-gated and **skips** unless `PiuTest:*` / `Discord:*` secrets are set (the shared AppHost store), so it is inert by default. Named in no `azure-pipelines.yml` `dotnet test` step (never runs in CI) and Sonar-excluded; it builds with the solution only to stay compile-honest. Not a feature suite — a formula that must be guaranteed belongs in `ScoreTracker.Tests` or `Tests.Integration`.
...
| `ScoreTracker.ExplorationTests/` | Exploration (manual only — never CI, never a feature guarantee) | — | Real external sites (live PIU crawl, real Discord). Config-gated, skips unless secrets set. `LiveSite/` + `DiscordCanary/`. Read-only by default; mutate only on the owner's explicit ask |
