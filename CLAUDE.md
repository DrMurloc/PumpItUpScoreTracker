...
- **Exploration workbench (manual only, NOT CI, NOT feature testing)**: `dotnet test ScoreTracker/ScoreTracker.ExplorationTests/ScoreTracker.ExplorationTests.csproj` — a sanctioned place to crawl live pages (read-only unless the owner explicitly asks for a mutation), download files, and try formulas against real data/images (OCR experiments land here). Holds the PIU live-site probes (`LiveSite/`, `[LiveSiteFact]`), the Discord card canary (`DiscordCanary/`, `[DiscordCanaryFact]`) — both moved out of `Tests.Integration` — the catalog probes (`Catalog/`, `[CatalogProbeFact]`), which assert hardcoded domain data (e.g. every chart title's song/type/level) resolves against a real populated catalog, something CI's empty migrated schema can never do, and the translation workbench (`Translations/`, `[TranslationProbeFact]`), the only place in the solution that calls a metered LLM. Every test is config-gated and **skips** unless `PiuTest:*` / `Discord:*` / `CatalogProbe:ConnectionString` / `ClaudeApi:ApiKey` secrets are set (the shared AppHost store), so it is inert by default. Named in no `azure-pipelines.yml` `dotnet test` step (never runs in CI) and Sonar-excluded; it builds with the solution only to stay compile-honest. Not a feature suite — a formula that must be guaranteed belongs in `ScoreTracker.Tests` or `Tests.Integration`.
...
| `ScoreTracker.Data` | + `Microsoft.EntityFrameworkCore.SqlServer`, `Azure.Storage.Blobs`, `Discord.Net`, `HtmlAgilityPack`, `SendGrid`, `SkiaSharp` (+ Linux native assets) + `QRCoder` (the share-card renderer) + `Azure.Security.KeyVault.Keys` / `Azure.Identity` (the remembered-credential KEK envelope, `IKeyEnvelope`) | ASP.NET, and `Microsoft.Extensions.Http` — `DevTooling/` reads the live site's `api/v2` on a plain `HttpClient` it owns, because a factory client carries ServiceDefaults' 30-second resilience timeout and a bulk download outlives it. |
...
| Vertical assemblies: `ScoreTracker.ScoreLedger`, `ScoreTracker.OfficialMirror`, `ScoreTracker.Catalog`, `ScoreTracker.Randomizer`, `ScoreTracker.ChartIntelligence`, `ScoreTracker.ChartComments`, `ScoreTracker.WeeklyChallenge`, `ScoreTracker.EventCompetition`, `ScoreTracker.Communities`, `ScoreTracker.CommunityTools`, `ScoreTracker.PlayerProgress`, `ScoreTracker.Identity` (WeeklyChallenge is the template) | `MediatR`, `MassTransit.Abstractions` (+ full `MassTransit` for verticals with bus consumers — `IRegistrationConfigurator`, used by the `AddXxxConsumers` hooks, lives in the full package), `Microsoft.Extensions.DependencyInjection.Abstractions`, `Microsoft.Extensions.Caching.Memory`; `OfficialMirror` additionally `Microsoft.Extensions.Http` + `HtmlAgilityPack` (the PiuGame ACL), and `CommunityTools` additionally `Microsoft.Extensions.Http` (its own typed outbound webhook client — same exemption, a vertical that talks to a remote owns its client); `ChartComments` adds nothing new to the solution, and additionally references `Communities` + `CommunityTools` for the audience list and the link allowlist (+ project refs `Domain`, `Data` transitionally for the shared DbContext; only `Randomizer` still refs `Application` transitionally (`GetRandomChartsQuery` — unpinning it is a move, not a deletion; see Known divergences); cross-vertical reads go through published ports (`IScoreReader`, `IPlayerStatsReader`, `ITitleRepository`, `IUserReader`, `IPlayerVisibilityReader` — who may look at whom, implemented in Rivals for now, see [docs/design/peers-abstraction.md](docs/design/peers-abstraction.md)), never SQL joins; a Domain port breaks a cycle the other way — `ISessionDeliveryClient` is how OfficialMirror hands a piugame session to CommunityTools) | Everything else. Only `Contracts/` and `Wiring/` namespaces may be public (arch-test enforced, `VerticalBoundaryTests`); EF entities and internal domain types never cross the boundary. Verticals must not be referenced by `Application` (cycles through `Data → Application`). **MassTransit's `AddConsumers` scan skips internal types** — verticals with bus consumers expose an `AddXxxConsumers(IRegistrationConfigurator)` hook called from `Program.cs`'s `AddMassTransit` block (tripwire-tested). |
...
| `ScoreTracker.Tests.Integration` | + `xunit`, `Moq`, `Testcontainers.MsSql`, `Respawn`, `WireMock.Net` (a real HTTP listener is a real dependency — the outbound webhook client's whole job is classifying what a remote did back) | Mocking the port whose implementation is the subject of the test (e.g., do not mock `IPhoenixRecordRepository` when testing `EFPhoenixRecordsRepository`). Mocking incidental collaborator ports for setup — or `IPiuGameApi`-style external clients in slice tests — is fine. |
...
| `ScoreTracker.ExplorationTests` | + `xunit`, `Moq`, `Anthropic` (the translation workbench is the only place in the solution that calls a metered LLM — the `ILanguageModelClient` Domain port has no implementation in `Data` and no DI registration, so nothing that ships can reach one) (no Testcontainers/Respawn — talks to real external sites, not an ephemeral DB); references `CompositionRoot` + `OfficialMirror` (direct, for the internal `PiuGameApi`) + the verticals | The manual-only crawl/download/formula workbench (see the commands section). It talks to real remote systems, so it is **read-only by default** — a run mutates a remote (POST/PUT/DELETE) only when the owner explicitly asks; the Discord canary posting to the owner's own lab channel is the one standing exception. Never CI, never a feature guarantee. Grant it `InternalsVisibleTo` from a vertical only when a probe genuinely needs an internal type (currently just `OfficialMirror`). |
...
- **Tables are never dropped — they are archived** (owner standard, 2026-07-27). When a feature is deleted, its EF entity and `ToTable` line go, but the rows stay queryable: the scaffolded migration's `DropTable` is hand-replaced with `ALTER SCHEMA archive TRANSFER scores.<Table>`. A revived feature then starts from real data instead of nothing. `archive` is the **only** sanctioned destination — the `back` / `bup` / `books` / `smx` schemas are unrelated legacy artifacts, and the older `*_archived`-suffix-in-`scores` convention is superseded. Archived tables move to the "Archived" section of [docs/DATABASE-SCHEMA.md](docs/DATABASE-SCHEMA.md) rather than being deleted from it.
- **Never edit an applied migration** (or its `.Designer.cs`). `__EFMigrationsHistory` says it already ran, so an edit changes nothing in production while silently altering every build-from-scratch environment (Testcontainers integration, E2E, fresh dev DBs) that replays the chain. Schema changes are always a **new** migration; hand-editing that new, unapplied migration is normal. `ChartAttemptDbContextModelSnapshot.cs` is the one snapshot that regenerates.

## UI conventions

The machine-enforceable subset of [docs/UX-GUIDELINES.md](docs/UX-GUIDELINES.md):

- **No color literals in `Pages/`, `Components/`, or `Shared/`** — no hex strings, no MudBlazor `Colors.*` constants (arch-test enforced, `UiColorTokenTests`, shrink-only allowlist). A brand color belongs in a `MixPalette` (`Services/Theming/MixThemes.cs`); a color that carries data meaning belongs in a semantic token group with a `ThemeScales` accessor. Markup reads `var(--mix-*/--rarity-*/--diff-*/--plate-*/--slot-*/--judg-*/--life-*)`. The judgment (`--judg-*`) and lifebar-zone (`--life-*`) groups are mix-invariant like the alert colors — a MISS reads as a miss in every theme.
...
- **Localization**: every UI string goes through `L[…]`; new keys land in **all** locales in the same pass, **inserted in alphabetical position, never appended at the end** (arch-test enforced, `ResxKeysAreStoredAlphabetically`, `OrdinalIgnoreCase` — which also keeps `Min` adjacent to `{0} min` instead of a whole uppercase range apart). Appending put every branch's new keys on the same few lines before `</root>`, and a resx conflict there resolves badly: each side's last element is closed by the *shared* `</data>` after the conflict, so keeping both halves leaves an element unterminated. New keys follow each `docs/LOCALIZATION-<locale>.md` glossary (Mix: ko `시리즈`, ja `バージョン`, es/pt `versión`/`versão`, fr/it `Mix`). **Two keys must never differ only by case** (arch-test enforced, `LocalizationKeyTests`): MSBuild's `GenerateResource` deduplicates resx names *case-insensitively*, first-wins, and drops the loser from the compiled satellite entirely (MSB3568 "ignored"), so the second spelling resolves to nothing and every non-English locale silently renders English — while English looks perfect throughout, because the key text *is* the English copy. Reuse the existing casing, and when one label genuinely needs two registers (a column header vs. an inline phrase), give the inline one a placeholder key (`rank #{0}`, `{0} min`) rather than a case variant. The same ratchet checks every locale covers the same key set as `en-US` — **including `en-ZW`**, the Murloc joke locale, which reached full parity on 2026-07-28 and is no longer exempt. Murloc has its own ratchet too (`MurlocValuesUseOnlyTheMurlocAlphabet`): every value is built from the letters `a b g l m o p r u` and nothing else, because every batch that wrote it without a rule reached for vowel-mangled English (`Srglrch`, `Blrrgk`) or left the English untouched. Acronyms, `{0}` placeholders and a protected proper-noun list are exempt; the syllable inventory and term mappings live in [docs/LOCALIZATION-en-ZW.md](docs/LOCALIZATION-en-ZW.md).
- **Stylesheets and scripts ship under content-hashed names** (arch-test enforced, `StaticAssetVersioningTests`). A `<link>`/`<script>` for app-owned CSS/JS reads `@Assets["css/site.css"]`, never a raw `/css/site.css` and never a hand-bumped `?v=3` — `MapStaticAssets` publishes the hashed name, so a release that changes a file changes its URL and no browser can serve the old one back. ES modules pulled in through `JSRuntime` `"import"` need no call-site change: `<ImportMap />` in `App.razor` rewrites the specifier. Two exemptions: `_framework/blazor.web.js` (the framework versions its own boot script), and **Razor Pages (`.cshtml`), where `@Assets` does not exist** — its collection is component-endpoint state, not a DI service, so injecting it 500s the page; those use `asp-append-version="true"`. See [docs/TECHNOLOGIES.md](docs/TECHNOLOGIES.md).
- **PUMBILITY is never rounded below the presentation layer** (arch-test enforced, `PumbilityPrecisionTests`, empty allowlist). A pool is fifty per-chart values that each carry a real fraction, so a value rounded early compounds its error and two surfaces that round at different points disagree about the same account — which is exactly how a session card came to read 17,195 while `/Pumbility` read 17,173. Doubles travel to the razor; there, **decimals are a PUMBILITY-section feature, not a sitewide one** (owner, 2026-08-09, reversing a one-day-old sitewide rule that cost layout everywhere): a pool total renders `ToString("N2")` on `/Pumbility`, `/Pumbility/Pool` and `/Pumbility/Phoenix1` — along with each chart's contribution there — and `ToString("N0")` on every other surface. A **gain** goes through `PumbilityFormat.Gain` or the `PumbilityDelta` component anywhere (whole at 10+, one decimal 1–10, two below 1, **truncated** at every rung and on the magnitude so a loss is not overstated either), and a **threshold** stays `N0` because it is an authored round number rather than a measurement. The Official Leaderboards pages keep their pre-existing `N2` — they quote piugame's own board. Storage is `float`. `Rating` is int-backed and is therefore never the type for a pool — `TotalRating` keeps it because the lifetime sum is not a pool.
- **No raw exception text outside `Pages/Admin/`** (arch-test enforced, `DiagnosticExposureTests`). Stack traces, framework error strings, and failed-job internals are maintainer data — they belong in the log and on an admin page, never on a player's screen or in an API response. What a user sees is a localized sentence they can act on; a **domain** exception's message is written to be exactly that and stays allowed. Contracts that carry raw exception text (`ImportRunRecord`, `GetImportRunsQuery`) are listed in the ratchet — add to that list when a new one appears. The maker-facing tool console is not an exception to this: a failed delivery reports a **closed vocabulary** (`Timeout`, `DnsFailure`, `TlsFailure`, `ClientError`, `ServerError`, `InvalidResponse`) plus the remote's own status code and body snippet, which is what a maker can act on and says nothing about our internals.
- **A PIUGame session is never written down** (arch-test enforced, `SessionModeNeverPersistsBodyTests`). An implementation of `ISessionDeliveryClient` must have no constructor or field dependency on anything that stores a delivery, and no `CommunityTools` entity may carry a `RedactedString` or a column named for a session. `RedactedString` is not protection at the persistence boundary — it masks `ToString()` but its JSON converter round-trips the real value, so serializing one into a table stores the credential in plaintext past a type that looks like it is guarding you. A conditional is not an acceptable fix for either half; removing the dependency is.
...
| `ScoreTracker.Tests.Integration/` | Integration | Medium | Ephemeral infra (SQL Server via Testcontainers). Pure real-DB repository/migration tests — the live-site and Discord-canary subfolders moved to `ScoreTracker.ExplorationTests` |
...
| `ScoreTracker.ExplorationTests/` | Exploration (manual only — never CI, never a feature guarantee) | — | Real external systems (live PIU crawl, real Discord, a populated prod-synced database). Config-gated, skips unless secrets set. `LiveSite/` + `DiscordCanary/` + `Catalog/` + `Translations/` (the one that spends money). Read-only by default; mutate only on the owner's explicit ask |
...
- **bUnit event seam** (arch-test enforced, `BunitEventDispatchTests`, shrink-only allowlist): a `Tests.Components` test that asserts on the result of an event makes itself `async Task` and awaits the dispatch — `await element.ClickAsync(new MouseEventArgs())` / `ChangeAsync` / `InputAsync` — never the synchronous `Click()`/`Change()`/`Input()`. The synchronous helpers **post** the event to the renderer's dispatcher and return without waiting; the test thread is not the dispatcher thread, and xUnit parks one collection per processor on a synchronous test body, so on a many-core box every pool worker is blocked at the pool minimum, the dispatch waits on the thread-injection throttle, and the assertion reads the pre-event render. It fails intermittently and points nowhere near its cause: nothing throws, the markup carries a real `blazor:onclick`, and the component's own state is simply unchanged — check `RenderCount` before hunting for a timer or a debounce, because there usually isn't one. `Tests.Components/ThreadPoolWarmup.cs` raises the pool's worker floor for the assembly, which holds the un-converted backlog at ordinary load; it is a mitigation, not a substitute — past roughly four concurrent copies of the suite the box is CPU-oversubscribed rather than thread-starved and no floor helps.
- **bUnit service seam**: a `Tests.Components` test stubs a dependency by adding a setup to the mock `ComponentTestBase` already registered — `Mediator`, `CurrentUser` — never by registering its own. bunit freezes the service provider the first time anything resolves from it, and a render does exactly that, so a later `Services.AddSingleton` throws instead of overriding. The exception names the registration line, not the render that closed the provider, which is why it reads as "this test is written wrong" rather than "this ran too late".
...
- **Account deletion is ratcheted four ways** (`AccountPurgeCoverageTests`, `ModelContributionRegistrationTests`, `AccountPurgeTests`): every entity with a `*UserId` column is named by some vertical's `UserOwned` manifest or by an exemption carrying its reason; every vertical `IConsumer<>` resolves from the host's real MassTransit registration; every `IDbModelContribution` is listed in `VerticalModelContributions.All()`; and every vertical's purge **runs** against a real migrated database, deleting every row it owns and no row it does not (`Tests.Integration/AccountPurgeTests` — a decoy account holds one probe row per manifest type, so purging a stranger must move nothing; the mocked handler suites cannot catch this, because a mock cannot over-delete). A vertical's purge manifest *is* its implementation — `UserDataPurge` executes the same array — so the two cannot drift. Naming is still not deleting, which is why the fourth exists: the manifest is a type list resolved to a column by convention at run time, so a table can be declared, pass the coverage ratchet, and throw the moment a purge reaches it. An entity carrying more than one Guid `*UserId` must say which one owns the row with `[PurgeKey(nameof(...))]` — the others point at different people (`CommunityMembership.GrantedByUserId` is the admin who granted the seat, not its holder), and purging on one of those deletes their data instead.
