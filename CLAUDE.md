...
| `ScoreTracker.Data` | + `Microsoft.EntityFrameworkCore.SqlServer`, `Azure.Storage.Blobs`, `Discord.Net`, `HtmlAgilityPack`, `SendGrid`, `SkiaSharp` (+ Linux native assets) + `QRCoder` (the share-card renderer) + `Azure.Security.KeyVault.Keys` / `Azure.Identity` (the remembered-credential KEK envelope, `IKeyEnvelope`) + `Microsoft.Extensions.Http` (`DevTooling/` rebuilds a local database by reading the live site's `api/v2` like any integrator) | ASP.NET. |
...
- **PUMBILITY is never rounded below the presentation layer** (arch-test enforced, `PumbilityPrecisionTests`, empty allowlist). A pool is fifty per-chart values that each carry a real fraction, so a value rounded early compounds its error and two surfaces that round at different points disagree about the same account — which is exactly how a session card came to read 17,195 while `/Pumbility` read 17,173. Doubles travel to the razor; there, a **total** renders `ToString("N2")` (what piugame prints), a **gain** goes through `PumbilityFormat.Gain` or the `PumbilityDelta` component (whole at 10+, one decimal 1–10, two below 1, **truncated** at every rung and on the magnitude so a loss is not overstated either), and a **threshold** stays `N0` because it is an authored round number rather than a measurement. Storage is `float`. `Rating` is int-backed and is therefore never the type for a pool — `TotalRating` keeps it because the lifetime sum is not a pool.
