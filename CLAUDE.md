...
| `ScoreTracker.ExplorationTests` | + `xunit`, `Moq`, `Anthropic` (the translation workbench keeps the only implementation of the synchronous `ILanguageModelClient` port — never registered in the app; the shipping path is `Data`'s `ILanguageModelBatchClient` adapter, which parks itself without a `ClaudeApi:ApiKey`, so spending a metered token requires configuration, never just code) (no Testcontainers/Respawn — talks to real external sites, not an ephemeral DB); references `CompositionRoot` + `OfficialMirror` (direct, for the internal `PiuGameApi`) + the verticals | The manual-only crawl/download/formula workbench (see the commands section). It talks to real remote systems, so it is **read-only by default** — a run mutates a remote (POST/PUT/DELETE) only when the owner explicitly asks; the Discord canary posting to the owner's own lab channel is the one standing exception. Never CI, never a feature guarantee. Grant it `InternalsVisibleTo` from a vertical only when a probe genuinely needs an internal type (currently just `OfficialMirror`). |
