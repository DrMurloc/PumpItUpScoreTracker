...
- **Exploration workbench (manual only, NOT CI, NOT feature testing)**: `dotnet test ScoreTracker/ScoreTracker.ExplorationTests/ScoreTracker.ExplorationTests.csproj` — a sanctioned place to crawl live pages (read-only unless the owner explicitly asks for a mutation), download files, and try formulas against real data/images (OCR experiments land here). Holds the PIU live-site probes (`LiveSite/`, `[LiveSiteFact]`), the Discord card canary (`DiscordCanary/`, `[DiscordCanaryFact]`) — both moved out of `Tests.Integration` — and the catalog probes (`Catalog/`, `[CatalogProbeFact]`), which assert hardcoded domain data (e.g. every chart title's song/type/level) resolves against a real populated catalog, something CI's empty migrated schema can never do. Every test is config-gated and **skips** unless `PiuTest:*` / `Discord:*` / `CatalogProbe:ConnectionString` secrets are set (the shared AppHost store), so it is inert by default. Named in no `azure-pipelines.yml` `dotnet test` step (never runs in CI) and Sonar-excluded; it builds with the solution only to stay compile-honest. Not a feature suite — a formula that must be guaranteed belongs in `ScoreTracker.Tests` or `Tests.Integration`.
...
| `ScoreTracker.ExplorationTests` | + `xunit`, `Moq` (no Testcontainers/Respawn — talks to real external sites, not an ephemeral DB); references `CompositionRoot` + `OfficialMirror` (direct, for the internal `PiuGameApi`) + the verticals | The manual-only crawl/download/formula workbench (see the commands section). It talks to real remote systems, so it is **read-only by default** — a run mutates a remote (POST/PUT/DELETE) only when the owner explicitly asks; the Discord canary posting to the owner's own lab channel is the one standing exception. Never CI, never a feature guarantee. Grant it `InternalsVisibleTo` from a vertical only when a probe genuinely needs an internal type (currently just `OfficialMirror`). |
